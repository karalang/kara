...
///
/// - `/ws` WebSocket route — phase-8 line 62 not shipped.
/// - `std.tracing` span emission on each handler call — phase-8
///   line 65 partial (data shapes shipped, full plumbing not).
///   The generated source uses a single `println` for "listening"
///   announcement so operators still get a stdout signal.
///
/// Both join the scaffold the moment their underlying surface ships.
