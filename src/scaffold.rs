...
///
/// What ships: `std.http` `Server.serve` on `127.0.0.1:8080`,
...
///
/// What's deferred — explicitly absent from the v1 scaffold so the
/// generated source compiles today against the shipped stdlib floor:
///
/// - `/ws` WebSocket route — needs a `Server.serve` upgrade hook that
///   does not exist yet; the shipped WS surface is a standalone accept
///   loop (`examples/ws_idle_holder`), not a `Server.serve` handler arm.
/// - Per-handler `std.tracing` spans — the ambient/registered exporter
///   that would let a handler emit without threading an exporter value is
///   a separate deferred item; the scaffold emits one startup event
///   through an explicit `StdoutExporter`, the v1 caller-plumbed idiom.
///
/// Both deepen the moment their underlying surface ships.
fn backend_main_body() -> String {
    "// Backend HTTP server skeleton, generated by `karac new`.\n\
     //\n\
     // Bind on 127.0.0.1:8080 and serve a `/health` endpoint that returns\n\
     // `200 OK / ok`. Every other path falls through to a 404. Replace\n\
     // `handle` with your own dispatch as the project grows.\n\
     //\n\
     ...
     let _result = Server.serve(addr, handle);\n\
     }\n"
    .to_string()
}
...
mod tests {
    ...
    fn backend_main_body_carries_required_anchors() {
        ...
        // std.http server entry point.
        assert!(
            body.contains("Server.serve("),
            "backend main must call Server.serve; body was:\n{body}"
        );
    ...
    fn scaffold_backend_writes_main_kara_and_manifest() {
        ...
        assert!(main.contains("Server.serve("));
