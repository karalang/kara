...
/// same per-element Display fn the identifier path uses. The variable case is
/// name-addressed (`var_elem_type_exprs`, set at the `let` binding); this is
...
/// Surface `TypeExpr` for every expression whose Kāra type is one of the three
/// SEQUENCE shapes — `Vec[T]`, `Slice[T]` (incl. `mut Slice[T]`) and
/// `Array[T, N]` — keyed by `(span.offset, span.length)`. The operand oracle
/// for the bare `==` / `!=` intercept in codegen (B-2026-08-27-10 for `Vec`,
/// B-2026-08-27-24 / -25 for the two siblings).
///
/// The FULL type is recorded, not just the element, because the three shapes
/// need three different comparators and codegen cannot tell them apart from
/// the LLVM value alone: a `Vec` arrives as a 3-field struct that is
/// indistinguishable from a `String`, a `Slice` as a bare `ptr` that is
/// indistinguishable from a `shared struct` handle, and only `Array` has a
/// self-describing `[N x T]` shape. Dispatching on the recorded surface type
/// is what makes the intercept safe to place ahead of `compile_binop`.
///
/// A `ref Vec[T]` / `mut ref Vec[T]` operand records the SAME type as an owned
/// one: the typechecker's `Eq` arm auto-derefs both sides
...
/// borrow does not own.
pub type SeqEqOperandTypesTable = std::collections::HashMap<(usize, usize), TypeExpr>;
...
pub struct Program {
    ...
    /// `TypeExpr` per `Vec[T]` / `Slice[T]` / `Array[T, N]`-typed expression
    /// (borrows peeled). Lets the `==` / `!=` intercept route a bare sequence
    /// operand to its content comparator instead of `compile_binop`'s
    /// field-count dispatch, which mistook a `Vec` for a `String`, or its
    /// pointer / non-comparable arms, which refused `Slice` and `Array`
    /// outright. See [`SeqEqOperandTypesTable`].
    pub seq_eq_operand_types: SeqEqOperandTypesTable,
    /// Set by the lowering pass from `TypeCheckResult.expr_types`: surface
