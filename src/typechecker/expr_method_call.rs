...
//! inference.
//!
//! `infer_method_call` is a **first-match-wins chain** of receiver/method
//! guards, and it is being decomposed one family at a time into sibling
//! `method_*.rs` modules — see
//! [`docs/spikes/state-decomposition-codegen-methodcall.md`]. Each extracted
//! family is called from the exact position in the chain its blocks
//! occupied, so **both** the position of those calls and the block order
//! inside each extracted module are load-bearing. Already moved out:
//! `method_numeric.rs` (scalar primitives), `method_vec_mutation.rs`
//! (`Vec`/`VecDeque` mutators).
//!
...
impl<'a> super::TypeChecker<'a> {
    ...
    pub(super) fn rerecord_resolved_ctor_arg(&mut self, arg: &crate::ast::Expr, resolved: &Type) {
        let is_ctor = matches!(
            &arg.kind,
    ...
    pub(super) fn bound_element_for_trait(
        &mut self,
        type_param_name: &str,
        trait_name: &str,
    ) -> Option<Type> {
    ...
    pub(super) fn receiver_type_param_name(obj_ty: &Type) -> Option<String> {
        match obj_ty {
    ...
    pub(super) fn dispatch_typeparam_receiver_method(
        &mut self,
        type_param_name: &str,
        method: &str,
        args: &[CallArg],
    ...
    pub(super) fn dispatch_self_receiver_method(
        &mut self,
        method: &str,
        args: &[CallArg],
        span: &Span,
    ...
    pub(super) fn infer_method_call(
        ...
        // Identifier-receiver calls — the `ptr` provenance module, the other
        // prelude module paths, and type-receiver associated calls
        // (`T.method(..)`, including `From` disambiguation). Extracted to
        // `method_identifier_receiver.rs`; it keeps this position in the
        // first-match-wins chain and the block order within it.
        if let Some(t) = self.try_identifier_receiver_method(object, method, args, span) {
            return t;
        }

        // Concrete-type UFCS dispatch — `TypeName[T1, T2, ...].method(args)`.
        // Concrete-type UFCS — `TypeName[Args].method(..)`, which the
        // parser gives us as a single-segment `Path` receiver with explicit
        // generic args. Extracted to `method_identifier_receiver.rs`; it
        // keeps this position in the first-match-wins chain.
        if let Some(t) = self.try_path_receiver_method(object, method, args, span) {
            return t;
        }
        ...
        // first-match-wins chain and the block order within it.
        if let Some(t) = self.try_pointer_receiver_method(object, method, args, span, &obj_ty) {
            return t;
        }
        ...
        // Fresh-temporary receiver type recording for codegen — the
        // span-keyed `temp_recv_*` side tables that let codegen reconstruct
        // a temporary receiver's shape (`make_vec().iter()`, `[1,2].iter()`,
        ...
        // Slice views and raw-pointer conversions on sequence receivers —
        ...
        // `Vector[T, N]` portable-SIMD instance methods (design.md
        // § Portable SIMD). Extracted to `method_simd.rs`; it keeps this
        // position in the first-match-wins chain and the block order
        // within it.
        if let Some(t) = self.try_simd_vector_method(method, args, span, &obj_ty) {
            return t;
        }

        // `Column` element-wise surface — `iter` / `iter_valid` / `fillna` /
        // `dropna`, `fold`, and `map` (with its Option/Result and Tensor
        ...
        // `Option` / `Result` combinators, closure-free and closure-taking
        ...
        // `Column` / `DataFrame` / `Tensor` analytics — `zip_with`, the
        // index/ordering reductions, the `Column` scalar reductions and the
        // `DataFrame` surface. Extracted to `method_column_analytics.rs`; it
        // keeps this position in the first-match-wins chain and the block
        // order within it.
        if let Some(t) =
            self.try_column_analytics_method(method, args, span, args_close_span, &obj_ty)
        {
            return t;
        }

        // Iterator / collection-aggregation / atomic surfaces — `iter`,
        // `iter_mut`, `into_iter`, collection `clone`, `sum`/`product`/
        // `max`/`min`, `join`/`concat`, the line iterators, and the atomic
        ...
        // `Vec` / `VecDeque` mutators that check the element slot (`push`,
        // `insert`, `extend`, `pop`, `remove`, `get_unchecked`, `push_back` /
        // `push_front`). Extracted to `method_vec_mutation.rs`; it keeps this
        // position in the first-match-wins chain and the block order within it.
        if let Some(t) = self.try_vec_mutation_method(method, args, &obj_ty) {
            return t;
        }
        ...
        // Named-container dispatch — `Map` / `Entry` / `SortedMap` /
        // `SortedSet` / `Set`, plus the `Regex`, `CStr` / `CString` and
        // HTTP named types. Extracted to `method_mapset.rs`; it keeps this
        // position in the first-match-wins chain and the block order
        // within it.
        if let Some(t) = self.try_mapset_method(method, args, span, &obj_ty_for_named) {
            return t;
        }
        ...
        // Sequence in-place mutation / reordering — the comparator sorts,
        // `retain`, `dedup`, `split_off`. Extracted to
        // `method_sequence_mutation.rs`; it keeps this position in the
        // first-match-wins chain and the block order within it.
        if let Some(t) = self.try_sequence_mutation_method(method, args, span, &obj_ty) {
            return t;
        }
        ...
        // it keeps this position in the first-match-wins chain and the
        // block order within it.
        if let Some(t) =
            self.try_nominal_tail_method(method, args, span, args_close_span, &receiver_for_lookup)
        {
            return t;
        }
        // Terminal arm: dispatch to a user `impl` block (candidate
        // collection, bound gating, specialization, and the `no method`
        // diagnostics). Extracted to `method_user_impl.rs`; reaching it is
        // the decision, so it returns `Type` rather than `Option<Type>`.
        self.dispatch_user_impl_method(object, method, args, span, &receiver_for_lookup)
    }
    ...
    pub(super) fn infer_vector_method(
        &mut self,
        element: &Type,
        lanes: &ConstArg,
        method: &str,
