...
use crate::ast::{BinOp, Block, Expr, ExprKind, PatternKind, Program, Stmt, StmtKind};
use crate::concurrency::{LoopReduction, ReductionOp};
// The auto-par cost model lives in a plain-data module so the analysis/query
...
impl<'ctx> super::Codegen<'ctx> {
    ...
    pub(super) fn expr_references_current_param(&self, expr: &Expr) -> bool {
        match &expr.kind {
            ExprKind::Identifier(n) => self.fn_ctx.current_fn_param_names.contains(n),
            ExprKind::Path { segments, .. } => segments
                .first()
                .is_some_and(|s| self.fn_ctx.current_fn_param_names.contains(s)),
    ...
    pub(super) fn try_emit_reduction_lowering(
        ...
        let Some(shape) = extract_loop_shape(parent_body, stmt_index, expr) else {
            return Ok(None);
        };
