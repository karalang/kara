...
pub(crate) struct RuntimeFns<'ctx> {
    ...
    /// `karac_string_slice_fail(data, len, start, end)` — the NORETURN failure
    /// half of `karac_string_slice_into`, called by the inline slice fast path
    /// when its own validation rejects the range. Declared `cold` + `noreturn`
    /// so the cold edge contributes no join, which is what lets SROA keep the
    /// fast path's descriptor in registers (B-2026-09-16-1).
    pub(crate) karac_string_slice_fail_fn: FunctionValue<'ctx>,
