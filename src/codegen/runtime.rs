...
impl<'ctx> super::Codegen<'ctx> {
    ...
    pub(super) fn zero_aggregate_field_caps(
        ...
        for i in 0..agg_ty.count_fields() {
            match agg_ty.get_field_type_at_index(i) {
                ...
                // B-2026-09-13-23 — a fixed `Array[T, N]` FIELD whose elements
                // own heap. This walker is LLVM-type-driven and had no
                // `ArrayType` arm, so a tuple holding `Array[String, 2]` was
                // moved with its source left fully armed. Harmless while the
                // array had no drop at all (it merely leaked); the moment the
                // tuple's drop learned to walk the array, the source and the
                // destination both freed the same buffers.
                //
                // An array has no cap word of its own — its storage is inline —
                // so the disarm is per ELEMENT, which is exactly what the
                // recursion below does: a `{ptr,len,cap}` element is zeroed by
                // the `vec_ty` arm above, and an aggregate element recurses.
                // That mirrors `emit_drop_fn_for_array`, whose per-element
                // drops all guard on `cap > 0`, so zeroing here makes the
                // source's whole array walk a no-op.
                Some(BasicTypeEnum::ArrayType(at)) => {
                    if let Ok(field_ptr) =
                        self.builder
                            .build_struct_gep(agg_ty, base_ptr, i, "movecap.arr")
                    {
    ...
    /// Zero the `cap` of every heap-owning ELEMENT of a fixed array
    /// (B-2026-09-13-23) — the array-shaped peer of
    /// [`Self::zero_aggregate_field_caps`], which it also recurses into for an
    /// aggregate element.
    ///
    /// An array has no cap word of its own: its storage is laid out inline, so
    /// the only disarmable state is per element. That mirrors
    /// `emit_drop_fn_for_array`, whose per-element drops each guard on
    /// `cap > 0`, so zeroing here makes the source's whole array walk a no-op
    /// and leaves the destination sole owner.
    ///
    /// Shared by BOTH move-suppression paths a tuple holding an array can take
    /// — the bare-tuple one through `zero_aggregate_field_caps` and the
    /// struct-field one through `zero_tuple_elem_cap_at` — because they are
    /// different branches reached by different shapes and each was measured to
    /// double-free on its own.
    pub(super) fn zero_array_elem_caps(
        &self,
        arr_ptr: PointerValue<'ctx>,
        at: inkwell::types::ArrayType<'ctx>,
    ) {
        let vec_ty = self.vec_struct_type();
        let i64_t = self.context.i64_type();
        ...
        for e in 0..at.len() {
            let ep = unsafe {
                self.builder.build_in_bounds_gep(
                    at,
                    arr_ptr,
                    &[i32_t.const_zero(), i32_t.const_int(e as u64, false)],
                    "movecap.arr.e",
                )
            };
            let Ok(ep) = ep else { continue };
            match elem_ty {
                BasicTypeEnum::StructType(est) if est == vec_ty => {
                    if let Ok(cap_ptr) =
                        self.builder
                            .build_struct_gep(vec_ty, ep, 2, "movecap.arr.cap")
                    {
                        let _ = self.builder.build_store(cap_ptr, i64_t.const_int(0, false));
                    }
                }
                BasicTypeEnum::StructType(est) => self.zero_aggregate_field_caps(ep, est),
                BasicTypeEnum::ArrayType(inner) => self.zero_array_elem_caps(ep, inner),
                _ => {}
            }
        }
    }

    /// True when an aggregate holds a fixed `Array[T, N]` field whose elements
    /// own heap — the narrow companion to [`Self::aggregate_has_heap_field`]
    /// for the one gate that must see through an `ArrayType` (B-2026-09-13-23).
    ///
    /// Deliberately NOT folded into `aggregate_has_heap_field`, which has 33
    /// call sites choosing copy depth and drop strategy across the backend;
    /// this answers only "does the move-out disarm have work to do here", which
    /// is the question `zero_aggregate_field_caps` above now knows how to act
    /// on.
