...
# universal invariant: B-2026-07-12-29 WAS an arm64-ONLY RC leak (a compound
# index-assign of a shared/Option[shared] Vec element balanced on x86 and leaked
# on arm64; fixed in 63d618e) — proof that this arm64 harness can flag a leak
# x86 CI's `memory-sanitizer` does NOT, and vice-versa. CI now has an arm64 leg
# (`memory-sanitizer-arm64`) covering the arm64-only class as a permanent gate.
# Reach for an x86 image (or trust x86 CI) to confirm whether a leak this
# harness reports is arch-specific.
set -euo pipefail
