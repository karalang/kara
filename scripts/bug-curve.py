...
def ledger_view(rows) -> str:
    """The human-readable rolled-up state — Open bugs in full, Fixed collapsed.
    ...
    regenerated by `--inject`; do not hand-edit the block it lands in.
    """
    total = len(rows)
    ...
    out.append(
        "<details><summary>"
        f"{len(fixed)} fixed — the regression test is the durable artifact; "
        "prose lives in each owning phase tracker</summary>\n"
    )
    ...
    for r in sorted(fixed, key=lambda r: r["date"]):
        fix = r.get("fix", "") or "—"
        out.append(
            f"| {r['id']} | {r['surface']} | {r['severity']} | {r['title']} "
            f"| {fix} |"
        )
    out.append("\n</details>")
    out.append("")
