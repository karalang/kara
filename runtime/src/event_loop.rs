...
/// Default `listen(2)` backlog for `karac_runtime_tcp_bind`. Linux's
/// kernel silently caps this at the system tunable (`somaxconn`), so
/// passing a large literal lets the OS pick the real ceiling without
/// the runtime needing to read it. 65535 matches what nginx / envoy
/// use by default. Surfaced at M3 1M (2026-05-29) — see phase-6-runtime.md
/// line 209 follow-on for the diagnosis (SYN-cookie fallback at ~93K
/// conns, 17% churn-timeout rate) and
/// `docs/investigations/demo1_m3_verification.json` for the validation
/// gate.
///
/// **macOS quirk (2026-05-29):** unlike Linux, Darwin does NOT clamp
/// `listen(backlog)` against `kern.ipc.somaxconn`. Passing
/// `listen(>16384)` on macOS leaves the listener in `LISTEN` state but
/// silently breaks loopback acceptance — incoming SYNs never get
/// SYN-ACK'd, so connecting clients hang in `connect()` until they
/// time out. Confirmed against pure-Python listeners (no Kāra in the
/// loop): `listen(16384)` accepts, `listen(32768)` doesn't.
/// 16384 is the largest documented-safe value we've observed; keep
/// it as the macOS ceiling so the cross-platform demos and tests
/// don't silently wedge on a Mac dev box.
#[cfg(target_os = "macos")]
