...
| `karac_runtime_event_loop_register_fd_cancel` (unix) | `extern "C" fn(raw_fd: i32, direction: u8, parked: *mut c_void, cancel: *const AtomicBool) -> u64` | Slice 5c. `register_fd` + a per-task `cancel` flag bound on the registration (`FdState.cancel`); the dispatcher / cancel-sweep hand it to `poll_fn`. The coroutine park-suspend reads the flag off its slot and passes it here; null = never-cancelled fallback. Requests a sweep if the flag is already set at register time (park-vs-cancel race guard). |
...
| `karac_runtime_request_cancel_sweep` | `extern "C" fn() -> i32` | Slice 5c. Request a cancel-sweep on every reactor shard and wake them (per-shard `sweep_requested` + waker). Called by `karac_runtime_taskgroup_cancel` after flipping child flags. No-op when the dispatcher isn't running. |
...
| `karac_runtime_tcp_try_clone` (unix + windows) | `extern "C" fn(fd: i64) -> i64` | Relay dogfood slice 3. Backs `TcpStream.try_clone(ref self) -> Result[TcpStream, TcpError]`. `dup(2)`s the socket fd (via `BorrowedFd::try_clone_to_owned`; Windows = `WSADuplicateSocket` via `BorrowedSocket::try_clone_to_owned`) so a connection splits into two owned handles for a full-duplex splice. Returns the new fd; -1 on failure. No parking (pure syscall, like connect/bind). |
| `karac_runtime_tcp_shutdown` (unix + windows) | `extern "C" fn(fd: i64, how: i64) -> i32` | Relay dogfood slice 3b. Backs `TcpStream.shutdown_write(ref self) -> Result[Unit, TcpError]` (called with how=1). `shutdown(2)` of one direction (0=Read, 1=Write, 2=Both) so a proxy propagates one side's EOF (a FIN) across a full-duplex splice. Reconstructs a `TcpStream` from the raw fd only to reach `shutdown`, then `into_raw_fd`s it (no close). 0 ok / -1 err. No parking. |
...
| `karac_runtime_taskgroup_cancel` | `unsafe extern "C" fn(group: *mut KaracTaskGroupHandle)` | A2 slice 5b-1 + 5c. Flip every registered child's per-task `cancel` flag, then `karac_runtime_request_cancel_sweep` so idle parked children are torn down. Backs the user-callable `TaskGroup.cancel()`. **Live** as of slice 5c (the dispatcher now routes the per-task flag — bound on the registration via `register_fd_cancel` — to parked coroutines, and the cancel-sweep reaches idle ones). |
...
- **Total `#[no_mangle]` exports (2026-05-26 audit):** 59.
  ...
  - 1 added 2026-06-18 (Relay dogfood slice 3): `karac_runtime_tcp_try_clone` (unix + windows) — `dup(2)`-backed second owned `TcpStream` handle (`TcpStream.try_clone`) enabling full-duplex splice (read-half + write-half tasks over one socket). Reached from codegen-emitted IR (`lower_tcp_stream_try_clone`).
  - 1 added 2026-06-18 (Relay dogfood slice 3b): `karac_runtime_tcp_shutdown` (unix + windows) — `shutdown(2)` half-close (`TcpStream.shutdown_write`) so a proxy propagates one side's EOF (a FIN) across a full-duplex splice. Reached from codegen-emitted IR (`lower_tcp_stream_shutdown_write`).
  ...
  - 1 added 2026-06-06 (A2 slice 5b-1): `karac_runtime_taskgroup_cancel` — cooperative cancellation, language surface + flag plumbing (`TaskGroup.cancel()`). Made live by slice 5c. (The 2026-05-26 audit total above predates the A2 spawn_coro / park_slot / taskgroup symbols and is a dated snapshot, not a live count.)
  - 4 added 2026-06-06 (A2 slice 5c trigger): `karac_runtime_event_loop_register_fd_cancel` (unix), `karac_runtime_request_cancel_sweep`, `karac_runtime_park_slot_bind_cancel`, `karac_runtime_park_slot_cancel_ptr` — per-task cancel routing (flag on the registration) + the dispatcher cancel-sweep for idle parked coroutines. `register_fd_cancel` + `park_slot_cancel_ptr` are reached from codegen-emitted IR (the coroutine park-suspend); `request_cancel_sweep` + `park_slot_bind_cancel` from internal runtime callers (`taskgroup_cancel` / `spawn_coro`).
