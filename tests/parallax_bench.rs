...
//!   names all six impls (kara, rust, go, node, phoenix, java). Pins
//!   script syntactic correctness without paying the bench cost in CI.
//!
...
mod parallax_bench_tests {
    ...
    /// `bench.sh --dry-run` exits 0 and lists the six impl names on
    ...
    fn test_bench_script_dry_run() {
        ...
        for impl_name in &["kara", "rust", "go", "node", "phoenix", "java"] {
            assert!(
