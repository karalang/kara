...
#[test]
fn interp_chained_optres_passthrough_owns_once() {
    assert_eq!(
        run(r#"struct R { id: i64, s: String }
impl Drop for R { fn drop(mut ref self) { println(f"d{self.id}") } }
struct N { id: i64, s: String }
...
#[test]
fn interp_handback_match_arm_moves_payload_out() {
    assert_eq!(
        run(r#"struct R { id: i64, s: String }
impl Drop for R { fn drop(mut ref self) { println(f"d{self.id}") } }
struct N { id: i64, s: String }
...
#[test]
fn interp_optres_handback_let_bound_branch_no_drop() {
    assert_eq!(
        run(r#"struct N { id: i64, s: String }
struct H { k: i64 }
impl H {
...
#[test]
fn interp_escaping_optres_param_handback_and_moves() {
    assert_eq!(
...
#[test]
fn interp_array_param_returned_inside_an_enum_variant_runs_its_bodies_once() {
    let out = run(r#"struct R { id: i64, s: String }
impl Drop for R { fn drop(mut ref self) { println(f"d{self.id}") } }
fn mkr(i: i64) -> R { return R { id: i, s: f"aaa" } }
...
/// at the `if`).
#[test]
fn interp_param_wrapped_in_a_local_and_handed_back_on_some_paths_runs_one_body() {
    let out = run(r#"shared struct Sh { k: i64 }
struct P { id: i64 }
impl Drop for P { fn drop(mut ref self) { println(f"dP{self.id}") } }
struct S2 { h: Sh, id: i64 }
...
#[test]
fn interp_param_handed_on_two_levels_to_a_mixed_path_callee_has_one_owner() {
    let out = run(r#"shared struct Sh { k: i64 }
struct S2 { h: Sh, id: i64 }
impl Drop for S2 { fn drop(mut ref self) { println(f"dS{self.id}") } }
...
#[test]
fn interp_tuple_destructure_array_leaf_runs_bodies_once() {
    let out = run_no_errors(
        r#"struct R { id: i64, s: String }
impl Drop for R { fn drop(mut ref self) { println(f"dR{self.id}") } }
...
#[test]
fn interp_recursive_hand_back_of_a_param_has_one_owner() {
    let out = run(r#"shared struct Sh { k: i64 }
struct S2 { h: Sh, id: i64 }
impl Drop for S2 { fn drop(mut ref self) { println(f"dS{self.id}") } }
struct S3 { h: Sh, id: i64 }
...
#[test]
fn interp_vec_in_option_or_map_field_runs_element_bodies() {
    let out = run_no_errors(
