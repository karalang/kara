...
///
/// The generic `G[T]` cell is left out here: it leaks one element at `-O0`
/// on the unfixed tree too, which is its own row.
#[test]
fn asan_vec_in_option_or_map_field_is_freed_once() {
    let round: &[&str] = &[
        "c1 true",
        "dD1",
        "dD2",
