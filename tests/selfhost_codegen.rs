...
const CORPUS: &[&str] = &[
    ...
    "fn main() { let mut v: Vec[String] = Vec.new(); v.push(\"keep\"); let s = v[0].clone(); println(s + \"-copy\"); println(v[0]) }",
    ...
    "#[derive(Clone)]\nstruct Token { kind: i64, text: String }\nfn main() { let mut toks: Vec[Token] = Vec.new(); toks.push(Token { kind: 1, text: \"let\" }); toks.push(Token { kind: 2, text: \"x\" + \"1\" }); let mut i = 0; while i < toks.len() { let t = toks[i].clone(); println(t.kind.to_string() + \":\" + t.text); i = i + 1; } }",
    ...
    "fn words() -> Vec[String] { let mut v: Vec[String] = Vec.new(); v.push(\"alpha\".to_string()); v.push(\"beta\".to_string()); return v; }\nfn total_len(v: Vec[String]) -> i64 { let mut n = 0; let mut i = 0; while i < v.len() { let w = v[i].clone(); n = n + w.len(); i = i + 1; } return n; }\nfn main() { println(total_len(words()).to_string()); let ws = words(); println(total_len(ws).to_string()) }",
    ...
    // Slice 39: STRUCT PAYLOADS IN ENUM VARIANTS — At(Span) /
    // Item(Named{String,i64}): the widened base + a struct slot 3
    // (one distinct struct payload type per enum), tag-gated slot-3
    // deep-copy/free legs, match binds the struct as a borrow, Vec[Ev]
    // strides account for the struct slot. Bind-before-ref-pass corpus
    // discipline (a heap TEMP passed to a ref param is a known leak
    // deferral, latent for strings too).
    "struct Span { line: i64, col: i64 }\nstruct Named { name: String, id: i64 }\n#[derive(Clone)]\nenum Ev { Nothing, At(Span), Count(i64), Label(String) }\nenum Rec { Empty, Item(Named) }\nfn show_ev(e: ref Ev) -> String {\n    match e {\n        At(sp) => {\n            let ln = sp.line;\n            let cl = sp.col;\n            return \"at \".to_string() + ln.to_string() + \":\" + cl.to_string();\n        }\n        Count(n) => {\n            return \"count \".to_string() + n.to_string();\n        }\n        Label(s) => {\n            return \"label \".to_string() + s;\n        }\n        Nothing => {}\n    }\n    return \"nothing\".to_string();\n}\nfn show_rec(r: ref Rec) -> i64 {\n    match r {\n        Item(nm) => {\n            return nm.id;\n        }\n        Empty => {}\n    }\n    return 0 - 1;\n}\nfn main() {\n    let a = Ev.At(Span { line: 3, col: 9 });\n    println(show_ev(a));\n    println(show_ev(Ev.Count(7)));\n    let lbl = Ev.Label(\"tick\".to_string());\n    println(show_ev(lbl));\n    println(show_ev(Ev.Nothing));\n    let b = a;\n    println(show_ev(b));\n    let r = Rec.Item(Named { name: \"alpha\".to_string(), id: 42 });\n    println(show_rec(r).to_string());\n    let r2 = r;\n    println(show_rec(r2).to_string());\n    println(show_rec(Rec.Empty).to_string());\n    let mut v: Vec[Ev] = Vec.new();\n    v.push(Ev.At(Span { line: 1, col: 1 }));\n    v.push(Ev.Count(5));\n    let mut i = 0;\n    while i < v.len() {\n        let e = v[i].clone();\n        match e {\n            At(sp) => {\n                let ln = sp.line;\n                println(\"v at \".to_string() + ln.to_string());\n            }\n            Count(n) => {\n                println(\"v count \".to_string() + n.to_string());\n            }\n            Label(s) => {\n                println(s);\n            }\n            Nothing => {\n                println(\"v nothing\");\n            }\n        }\n        i = i + 1;\n    }\n}",
    // Slice 40: CALLER FREES HEAP TEMPS LENT TO ref PARAMS — a heap temp
    // (concat result / constructor) passed to a `ref` param is owned by
    // no one (callee non-owning), so the CALLER frees it post-call.
    // emit_ref_temp_frees, kind-dispatched, at all three call sites
    // (value/statement/method). Closes the Slice-39 deferral — inline
    // temps to ref params no longer need bind-first discipline.
    "struct Named { name: String, id: i64 }\nenum Ev { Nothing, Count(i64), Label(String) }\nfn show_ev(e: ref Ev) -> String {\n    match e {\n        Count(n) => {\n            return \"count \".to_string() + n.to_string();\n        }\n        Label(s) => {\n            return \"label \".to_string() + s;\n        }\n        Nothing => {}\n    }\n    return \"nothing\".to_string();\n}\nfn tag(n: ref Named) -> String {\n    return n.name;\n}\nfn wrap(s: ref String) -> String {\n    return \"[\".to_string() + s + \"]\";\n}\nfn total(v: ref Vec[i64]) -> i64 {\n    let mut t = 0;\n    let mut i = 0;\n    while i < v.len() {\n        t = t + v[i];\n        i = i + 1;\n    }\n    return t;\n}\nfn build() -> Vec[i64] {\n    let mut v: Vec[i64] = Vec.new();\n    v.push(3);\n    v.push(4);\n    return v;\n}\nfn main() {\n    println(show_ev(Ev.Label(\"tick\".to_string())));\n    println(show_ev(Ev.Count(9)));\n    println(tag(Named { name: \"alpha\".to_string(), id: 1 }));\n    println(wrap(\"mid\".to_string() + \"dle\"));\n    println(total(build()).to_string());\n}",
    "struct Named { name: String, id: i64 }\nenum Ev { Nothing, At(i64), Label(String) }\nfn show_ev(e: ref Ev) -> String {\n    match e {\n        At(n) => {\n            return \"at \".to_string() + n.to_string();\n        }\n        Label(s) => {\n            return \"label \".to_string() + s;\n        }\n        Nothing => {}\n    }\n    return \"nothing\".to_string();\n}\nfn main() {\n    println(show_ev(Ev.Label(\"tick\".to_string())));\n    println(show_ev(Ev.At(5)));\n}",
    // Slice 41: VEC PAYLOADS IN ENUM VARIANTS — Nums(Vec[i64]): the
    // en_extra slot-3 generalized to any single aggregate kind (struct
    // OR vec) via en_extra_stride/has_heap/materialize/free dispatch
    // helpers. Construction routes vec args to slot 3, match binds the
    // vec as a borrow, Vec[Bag] strides account for it. One aggregate
    // TYPE per enum (both variants Vec[i64]); mixed String+Vec-payload
    ...
    // Slice 44: f64 ENUM PAYLOADS — Real(f64) / Float(f64, String), the
    // last token.kara payload shape. Stored as BITS in the i64 slot 1
    // (bitcast f64->i64 on construction, i64->double on match); no new
    // slot, tag-disjoint with i64 payloads. Composes with the two-payload
    // machinery (Float's f64 -> slot 1 bits, String -> slot 2).
    "enum Num { Zero, Real(f64), Float(f64, String) }\nfn describe(n: ref Num) -> String {\n    match n {\n        Real(x) => {\n            return \"real \".to_string() + x.to_string();\n        }\n        Float(y, lex) => {\n            return \"float \".to_string() + y.to_string() + \" '\" + lex + \"'\";\n        }\n        Zero => {}\n    }\n    return \"zero\".to_string();\n}\nfn dbl(n: ref Num) -> f64 {\n    match n {\n        Real(x) => {\n            return x * 2.0;\n        }\n        Float(y, lex) => {\n            return y + 1.0;\n        }\n        Zero => {}\n    }\n    return 0.0;\n}\nfn main() {\n    let a = Num.Real(3.5);\n    println(describe(a));\n    println(dbl(a).to_string());\n    let b = Num.Float(0.1, \"0.1\".to_string());\n    println(describe(b));\n    println(dbl(b).to_string());\n    println(describe(Num.Zero));\n    let c = b;\n    println(describe(c));\n}",
    // Slice 45 CAPSTONE — THE RICH-TOKEN LEXER: the near-verbatim
    // token.kara/lexer.kara shape. A 13-variant Token enum with the FULL
    // payload variety — unit keywords/symbols, Ident(String),
    // Integer(i64, String), Float(f64, String) — SpannedToken { token,
    // span }, a Lexer with 6 methods scanning keywords/idents/integers/
    // FLOATS (fractional accumulation via i64.to_f64() + f64 arithmetic,
    // the bitcast-slot payload), symbol table, and render() matching all
    // 13 variants with span columns. Surfaced + fixed the to_f64 gap.
    "struct Span { line: i64, column: i64, offset: i64, length: i64 }\nenum Token {\n    Fn,\n    Let,\n    If,\n    LeftParen,\n    RightParen,\n    Plus,\n    Star,\n    Equal,\n    Dot,\n    Ident(String),\n    Integer(i64, String),\n    Float(f64, String),\n    Eof,\n}\nstruct SpannedToken { token: Token, span: Span }\nstruct Lexer { src: String, pos: i64, col: i64 }\nfn is_word_start(c: String) -> bool {\n    if c >= \"a\" and c <= \"z\" {\n        return true;\n    }\n    if c >= \"A\" and c <= \"Z\" {\n        return true;\n    }\n    return c == \"_\";\n}\nfn is_word_cont(c: String) -> bool {\n    if is_word_start(c) {\n        return true;\n    }\n    return c >= \"0\" and c <= \"9\";\n}\nfn is_digit(c: String) -> bool {\n    return c >= \"0\" and c <= \"9\";\n}\nfn digit_val(c: String) -> i64 {\n    if c == \"1\" {\n        return 1;\n    }\n    if c == \"2\" {\n        return 2;\n    }\n    if c == \"3\" {\n        return 3;\n    }\n    if c == \"4\" {\n        return 4;\n    }\n    if c == \"5\" {\n        return 5;\n    }\n    if c == \"6\" {\n        return 6;\n    }\n    if c == \"7\" {\n        return 7;\n    }\n    if c == \"8\" {\n        return 8;\n    }\n    if c == \"9\" {\n        return 9;\n    }\n    return 0;\n}\nfn keyword_of(w: ref String) -> i64 {\n    if w == \"fn\" {\n        return 1;\n    }\n    if w == \"let\" {\n        return 2;\n    }\n    if w == \"if\" {\n        return 3;\n    }\n    return 0;\n}\nfn kw_token(k: i64) -> Token {\n    if k == 1 {\n        return Token.Fn;\n    }\n    if k == 2 {\n        return Token.Let;\n    }\n    return Token.If;\n}\nfn sym_token(c: ref String) -> Token {\n    if c == \"(\" {\n        return Token.LeftParen;\n    }\n    if c == \")\" {\n        return Token.RightParen;\n    }\n    if c == \"+\" {\n        return Token.Plus;\n    }\n    if c == \"*\" {\n        return Token.Star;\n    }\n    if c == \"=\" {\n        return Token.Equal;\n    }\n    return Token.Dot;\n}\nimpl Lexer {\n    fn new(src: String) -> Lexer {\n        return Lexer { src: src, pos: 0, col: 1 };\n    }\n    fn done(ref self) -> bool {\n        return self.pos >= self.src.len();\n    }\n    fn peek(ref self) -> String {\n        return self.src.substring(self.pos, self.pos + 1);\n    }\n    fn advance(mut ref self) {\n        self.pos = self.pos + 1;\n        self.col = self.col + 1;\n    }\n    fn skip_ws(mut ref self) {\n        while self.pos < self.src.len() and self.peek() == \" \" {\n            self.advance();\n        }\n    }\n    fn next(mut ref self) -> SpannedToken {\n        self.skip_ws();\n        let start = self.pos;\n        let start_col = self.col;\n        if self.pos >= self.src.len() {\n            return SpannedToken {\n                token: Token.Eof,\n                span: Span { line: 1, column: start_col, offset: start, length: 0 },\n            };\n        }\n        if is_word_start(self.peek()) {\n            while self.pos < self.src.len() and is_word_cont(self.peek()) {\n                self.advance();\n            }\n            let w = self.src.substring(start, self.pos);\n            let ln = self.pos - start;\n            let k = keyword_of(w);\n            if k > 0 {\n                return SpannedToken {\n                    token: kw_token(k),\n                    span: Span { line: 1, column: start_col, offset: start, length: ln },\n                };\n            }\n            return SpannedToken {\n                token: Token.Ident(w),\n                span: Span { line: 1, column: start_col, offset: start, length: ln },\n            };\n        }\n        if is_digit(self.peek()) {\n            let mut ival = 0;\n            while self.pos < self.src.len() and is_digit(self.peek()) {\n                ival = ival * 10 + digit_val(self.peek());\n                self.advance();\n            }\n            if self.pos < self.src.len() and self.peek() == \".\" {\n                self.advance();\n                let mut frac = 0.0;\n                let mut scale = 0.1;\n                while self.pos < self.src.len() and is_digit(self.peek()) {\n                    let dv = digit_val(self.peek());\n                    frac = frac + scale * dv.to_f64();\n                    scale = scale * 0.1;\n                    self.advance();\n                }\n                let lex = self.src.substring(start, self.pos);\n                let fval = ival.to_f64() + frac;\n                return SpannedToken {\n                    token: Token.Float(fval, lex),\n                    span: Span { line: 1, column: start_col, offset: start, length: self.pos - start },\n                };\n            }\n            let lex = self.src.substring(start, self.pos);\n            return SpannedToken {\n                token: Token.Integer(ival, lex),\n                span: Span { line: 1, column: start_col, offset: start, length: self.pos - start },\n            };\n        }\n        self.advance();\n        let c = self.src.substring(start, self.pos);\n        return SpannedToken {\n            token: sym_token(c),\n            span: Span { line: 1, column: start_col, offset: start, length: 1 },\n        };\n    }\n}\nfn render(st: ref SpannedToken) -> String {\n    let mut head = \"?\".to_string();\n    match st.token {\n        Ident(name) => {\n            head = \"id:\".to_string() + name;\n        }\n        Integer(v, lex) => {\n            head = \"int:\".to_string() + v.to_string() + \"(\" + lex + \")\";\n        }\n        Float(f, lex) => {\n            head = \"float:\".to_string() + f.to_string() + \"(\" + lex + \")\";\n        }\n        Fn => {\n            head = \"kw:fn\".to_string();\n        }\n        Let => {\n            head = \"kw:let\".to_string();\n        }\n        If => {\n            head = \"kw:if\".to_string();\n        }\n        LeftParen => {\n            head = \"(\".to_string();\n        }\n        RightParen => {\n            head = \")\".to_string();\n        }\n        Plus => {\n            head = \"+\".to_string();\n        }\n        Star => {\n            head = \"*\".to_string();\n        }\n        Equal => {\n            head = \"=\".to_string();\n        }\n        Dot => {\n            head = \".\".to_string();\n        }\n        Eof => {\n            head = \"eof\".to_string();\n        }\n    }\n    let col = st.span.column;\n    return head + \"@\" + col.to_string();\n}\nfn main() {\n    let mut lx = Lexer.new(\"let x = 42 + y * 3.5\".to_string());\n    while not lx.done() {\n        let t = lx.next();\n        println(render(t));\n        lx.skip_ws();\n    }\n}",
    // Slice 46: `for x in <vec>` ELEMENT ITERATION — the idiom real
    ...
    // Slice 48: MATCH GUARDS — `Pat if <cond> =>`. Real parser code uses
    // guarded arms; the emitter had ignored the guard entirely (took the
    // first tag-matching arm). Now the guard is evaluated AFTER the
    // payload binds and, on failure, falls through to the next arm.
    // Wired into both match paths — enum-variant guards, guarded
    // wildcards, and int-literal-match guards.
    "enum Tok { Num(i64), Word(String), Sym }\nfn classify(t: ref Tok) -> String {\n    match t {\n        Num(n) if n > 100 => {\n            return \"big\".to_string();\n        }\n        Num(n) if n < 0 => {\n            return \"neg\".to_string();\n        }\n        Num(n) => {\n            return \"small \".to_string() + n.to_string();\n        }\n        Word(w) if w == \"if\" => {\n            return \"kw\".to_string();\n        }\n        Word(w) => {\n            return \"w \".to_string() + w;\n        }\n        Sym => {\n            return \"sym\".to_string();\n        }\n    }\n    return \"?\".to_string();\n}\nfn grade(n: i64) -> String {\n    match n {\n        x if x >= 90 => {\n            return \"A\".to_string();\n        }\n        x if x >= 80 => {\n            return \"B\".to_string();\n        }\n        _ => {\n            return \"F\".to_string();\n        }\n    }\n    return \"?\".to_string();\n}\nfn main() {\n    println(classify(Tok.Num(500)));\n    println(classify(Tok.Num(0 - 5)));\n    println(classify(Tok.Num(7)));\n    println(classify(Tok.Word(\"if\".to_string())));\n    println(classify(Tok.Word(\"foo\".to_string())));\n    println(classify(Tok.Sym));\n    println(grade(95));\n    println(grade(85));\n    println(grade(50));\n}",
    // Slice 49: OR-PATTERNS — `A | B | C =>`. The token-classifier idiom
    // real lexer/parser code uses (is_keyword/is_operator). The emitter
