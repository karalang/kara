...
/// The fifth postfix arm (B-2026-08-18-31), and the one the family's own
/// inventory missed: `?.` (B-2026-08-18-7), `?` (B-2026-08-18-9), `Index`
/// (B-2026-08-18-21) and `MethodCall` (B-2026-08-18-24) were the four rows,
/// and closing the fourth is what exposed this one — `receiver_struct_inst`
/// started answering for `v[0].second` when asked about `v[0]`, because those
/// two still shared a key.
#[test]
fn test_field_access_nodes_span_past_the_field_name() {
    let source = "fn f(p: Pair) -> i64 {\n  let a = p.inner.count;\n  return a;\n}";
    let prog = parse(source).program;
    ...
    assert_eq!(
        spans.len(),
        2,
        "expected two `FieldAccess` nodes, got {spans:?}"
    );

    let text = |s: &karac::token::Span| &source[s.offset..s.offset + s.length];
    // Outermost first: the whole projection, then the step it is built on.
    assert_eq!(text(&spans[0]), "p.inner.count");
    assert_eq!(text(&spans[1]), "p.inner");

    // The property every span-keyed side table relies on: each step of a
    // projection is its own key, distinct from the step below and the root.
    assert_ne!(
        (spans[0].offset, spans[0].length),
        (spans[1].offset, spans[1].length),
        "a chained field access must not share a SpanKey with its object"
    );
    assert_ne!(
        (spans[1].offset, spans[1].length),
        (spans[1].offset, 1),
        "a field access must not collapse onto its bare root `p`"
    );
}
...
fn a_128bit_literal_pattern_parses() {
    ...
    // `LiteralPattern::Integer` held an `i64` and genuinely could not represent
    // the value. (Before that diagnostic it PANICKED the compiler, which is
    // what B-2026-08-20-1 replaced.) The node now carries `i128`, matching
    // `ExprKind::Integer`, so the refusal is gone and the same literal parses
    // in pattern position and expression position alike.
    //
    // Both bounds are past `i64::MAX`, which is exactly where the old carrier
    // ran out: the `u128` case additionally rides its top half as a WRAPPED
    // negative, the same encoding the expression parser uses, so the two sides
    // compare as identical bits.
    for src in [
        ...
        // just past `i128::MAX` — a `u128` whose carrier value is negative
        "fn main() { let n: u128 = 170141183460469231731687303715884105728u128; match n { 170141183460469231731687303715884105728u128 => println(1), _ => println(2), } }",
        // as a RANGE bound, which goes through `parse_literal_pattern`
        "fn main() { let n: i128 = 5i128; match n { 1000000000000000000000000000000i128..=2000000000000000000000000000000i128 => println(1), _ => println(2), } }",
    ] {
        let parsed = parse(src);
        assert!(
            parsed.errors.is_empty(),
