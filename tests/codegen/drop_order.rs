...
#[test]
fn codegen_generic_multi_field_enum_array_payload_runs_element_bodies() {
    assert_eq!(
        run_program(
            r#"struct R { id: i64, s: String }
impl Drop for R { fn drop(mut ref self) { println(f"dR{self.id}") } }
...
#[test]
fn e2e_tuple_destructure_array_leaf_runs_bodies_on_every_backend() {
    assert_eq!(
        run_program(
            r#"struct R { id: i64, s: String }
impl Drop for R { fn drop(mut ref self) { println(f"dR{self.id}") } }
...
#[test]
fn e2e_struct_field_leaf_moved_out_of_an_arm_runs_its_body_once() {
    assert_eq!(
        run_program(
            r#"struct R { id: i64, tag: String, xs: Vec[i64] }
impl Drop for R { fn drop(mut ref self) { println(f"dR{self.id}") } }
