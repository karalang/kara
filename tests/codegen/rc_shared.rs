...
#[test]
fn e2e_param_wrapped_in_a_local_and_handed_back_runs_one_body() {
    let Some(out) = run_program(
        r#"shared struct Sh { k: i64 }
struct P { id: i64 }
impl Drop for P { fn drop(mut ref self) { println(f"dP{self.id}") } }
...
#[test]
fn e2e_dropless_struct_handed_back_in_an_option_frees_its_field_once() {
    let Some(out) = run_program(
        r#"shared struct Sh { k: i64 }
struct S3 { h: Sh, id: i64 }
enum Ho[T] { Full(T), Empty }
...
/// loop and a rebind.
#[test]
fn e2e_fresh_temp_handed_back_in_an_enum_frees_its_field_once() {
    let Some(out) = run_program(
        r#"shared struct Sh { k: i64 }
struct S2 { h: Sh, id: i64 }
impl Drop for S2 { fn drop(mut ref self) { println(f"dS{self.id}") } }
...
/// at the `if`).
#[test]
fn e2e_param_wrapped_in_a_local_and_handed_back_on_some_paths_runs_one_body() {
    let Some(out) = run_program(
        r#"shared struct Sh { k: i64 }
struct P { id: i64 }
impl Drop for P { fn drop(mut ref self) { println(f"dP{self.id}") } }
