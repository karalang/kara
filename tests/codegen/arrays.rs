...
#[test]
fn e2e_array_param_wrapped_by_the_callee_runs_its_element_bodies_once() {
    let Some(out) = run_program(
        r#"struct R { id: i64, s: String }
impl Drop for R { fn drop(mut ref self) { println(f"  d{self.id}") } }
...
/// any rebind.
#[test]
fn e2e_array_param_rebound_in_the_callee_is_a_view_of_the_callers_array() {
    let Some(out) = run_program(
        r#"struct R { id: i64, s: String }
impl Drop for R { fn drop(mut ref self) { println(f"  d{self.id}") } }
...
/// errors and 0 bytes in use at exit.
#[test]
fn e2e_array_param_handed_back_on_some_exits_is_dropped_once() {
    let Some(out) = run_program(
        r#"struct R { id: i64, s: String }
impl Drop for R { fn drop(mut ref self) { println(f"  d{self.id}") } }
fn mkr(i: i64) -> R { return R { id: i, s: f"heap-string-longer-than-sso-{i}" } }
...
#[test]
fn e2e_param_moved_into_local_through_branch_is_dropped_once() {
    let Some(out) = run_program(
        r#"struct R { id: i64, s: String }
impl Drop for R { fn drop(mut ref self) { println(f"  d{self.id}") } }
struct H { v: i64 }
...
/// that keeps nothing, and a dying call after a handing-back one). The compiled
/// twin of the interpreter fixture, pinning the side that was already right:
/// the interpreter's method frame adopted only struct and enum params, so it
/// ran none of them (`dies y8 d8 d9`) while every compiled surface ran both.
#[test]
fn e2e_method_array_param_dying_on_some_exits_runs_its_bodies() {
    let Some(out) = run_program(
        r#"struct R { id: i64, s: String }
impl Drop for R { fn drop(mut ref self) { println(f"  d{self.id}") } }
fn mkr(i: i64) -> R { return R { id: i, s: f"heap-string-longer-than-sso-{i}" } }
...
/// B-2026-09-23-20 — the compiled twin of
/// `interp_read_only_arm_over_array_payload_runs_its_bodies`: the compiled
/// backends were already right here, and this pins them to the same output the
/// interpreter now prints.
#[test]
fn e2e_read_only_arm_over_array_payload_runs_its_bodies() {
    let Some(out) = run_program(
        r#"struct R { id: i64, s: String }
impl Drop for R { fn drop(mut ref self) { println(f"  d{self.id}") } }
fn mkr(i: i64) -> R { return R { id: i, s: f"heap-string-longer-than-sso-{i}" } }
...
/// B-2026-09-23-23 — compiled twin: a SHADOWED owning `Array` local whose elements run a
...
#[test]
fn e2e_shadowed_array_local_returned_is_dropped_once() {
    let Some(out) = run_program(
        r#"struct R { id: i64, s: String }
impl Drop for R { fn drop(mut ref self) { println(f"  d{self.id}") } }
fn mkr(i: i64) -> R { return R { id: i, s: f"heap-string-longer-than-sso-{i}" } }
...
#[test]
fn e2e_param_pushed_into_a_container_on_some_paths_is_dropped_once() {
    let Some(out) = run_program(
        r#"struct R { id: i64, s: String }
impl Drop for R { fn drop(mut ref self) { println(f"d{self.id}") } }
fn mkr(i: i64) -> R { return R { id: i, s: f"aaa" } }
...
#[test]
fn e2e_array_param_returned_inside_an_enum_variant_is_dropped_once() {
    let Some(out) = run_program(
        r#"struct R { id: i64, s: String }
impl Drop for R { fn drop(mut ref self) { println(f"d{self.id}") } }
fn mkr(i: i64) -> R { return R { id: i, s: f"aaa" } }
