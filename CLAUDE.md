...
**AND THE IDENTITY ALONE CAN NEVER SAY A RUN WAS GREEN, which is the sharper half and the reason the skip count still has a job.** `passed + failed + ignored == the total` is pure libtest accounting: it closes whether every failure is an opt-in skip or every failure is a real regression, because either way each test was counted exactly once. Substitute one genuine failure for one pass and the identity closes at the same number. So it answers only "was any fixture silently ABSENT", and a gate that checks it and reports green is right exactly as long as every failure happens to be a skip — correct by coincidence, and silent until the first run where it matters. What distinguishes a green leg is a SECOND and independent assertion, `failed == the number of names under the SKIPPED heading`; when that one fails, some of those reds are real and the identity's closing tells you nothing about them.
...
**AND THE LEG'S OWN VERDICT LINE LIES TO A GREP**, which is why the identity and not the word is what to key on. The raw line reads `test result: FAILED. 1696 passed; 5 failed` — those 5 are the opt-in-archive fixtures the leg subtracts and then names, so the leg is GREEN and the word says FAILED. A reader grepping for `FAILED` gets a red that is not one; a script keying on it gets the expensive error. Measured 2026-09-20 on one such run: `1696 + 5 + 9 == 1710`, against 1710 `#[test]` in the file on that tree — `main` carried 1709 at the time and the leg's tree had one unlanded fixture, which is exactly why the count has to be taken from the tree the leg ran on rather than from a number written down here. A file-level identity proves the FILE ran; it says nothing about whether YOUR cell did. Only the named `<test path> ... ok` line proves that, so a run that added a fixture asserts both. **But that named line is not always AVAILABLE, and its absence is then a property of the OUTPUT FORMAT rather than evidence.** **NEITHER RATCHET LEG PRINTS PER-TEST LINES** — this paragraph said the `-O0` leg did until 2026-09-20, naming the one place the harvest demonstrably does not work as the place it does. `scripts/asan-o0-leg.sh` sends cargo's output to a `mktemp` log under `trap 'rm -f "$LOG"' EXIT` and emits only its own `>> `-prefixed summary lines, so the per-test lines are deleted before the leg returns. **A FAILING cell's captured output is the one exception, and it is a recent one**: until 2026-09-22 the leg named its new failures and then deleted the block that said WHY, so a red on this leg was a fixture name and nothing else — unreproducible by anyone the failure did not reproduce for, which is exactly the case that matters (B-2026-09-22-3: red on the CI runner in four of seven consecutive runs -- including a FAIL and a pass on a byte-identical test binary, so it is intermittent rather than a property of any tree or host -- green in two runs on a developer's box, and no report anywhere for any of them). It now echoes each new failure's captured block under a `CAPTURED OUTPUT` heading, capped at `ASAN_LEG_CAPTURE_LINES` lines for each of `ASAN_LEG_CAPTURE_MAX` failures and saying so when a cap bites. PASSING cells are still silent, so the arithmetic below remains the instrument for "did my cell run"; what changed is only that a red is now diagnosable where it was previously not. And `scripts/asan-instrumented-leg.sh` is not a second implementation to check separately — it sets three env vars and `exec`s that same script (its last line), so the two legs have identical output BY CONSTRUCTION, which is why a contrast drawn between them was wrong twice over. A wrapper that pipes a leg through `tail -25` removes the named lines just as effectively. **The named lines come from the `test-llvm` leg** — plain `cargo test --features llvm`, which runs `tests/memory_sanitizer.rs` as an ordinary test binary — at the DEFAULT opt level, so "does my cell pass at `-O0`" stays a SEPARATE question, asked as a filtered `-O0` run with a three-outcome verdict. Attribute a harvested line to its LEG and not to its LOG: a `grep -c` over a whole gate log cannot tell a line printed by the leg you meant from the same line printed by another, and here one of them runs at `-O0` and the other does not — `awk '/^=== LEG/{leg=$3} /<fixture> ok/{print leg": "$0}' gate.log` answers what the count silently guesses at. **And the leg's verdict reaches the caller PREFIXED**, as `>> test result: ...`, so a script anchoring `^test result:` matches nothing and reads as a leg that produced no verdict at all — which is also what a genuinely broken leg looks like, so narrow that refusal's cause before debugging the leg. On the ratchet legs the named-line check is UNAVAILABLE, not failing — which is why the arithmetic is the instrument to state for them; it is the only one left. Reading a missing name there as "my fixture did not run" is the same mistake as reading a silent instrument as a negative. Its virtue over comparing against the quarantine list is that it degrades in the SAFE direction as the lists drain rather than becoming more vacuous as they do, and it fails loudly on the one thing the comparison cannot see — fixtures that were silently absent from the run.
