...
- **Ownership handoff across the boundary (Slice 4) — `forget` shipped; round-trip blocked on two foundations.**
  ...
  - [ ] **Full allocate→use→free round-trip** — needs one of two foundations. **(a) Manual raw-pointer path**: blocked on raw-pointer instance methods `.offset`/`.read`/`.write` being spec'd-but-unimplemented in codegen (`B-2026-07-08-4`) — the lower-risk route. **(b) Auto-boxing sugar** (compiler auto-boxes a non-transparent return + auto-emits `karac_free_<T>`): invasive return-ABI surgery + a per-type drop-glue synthesizer, gated on the **ASAN/LSan differential fuzzer corpus** (`drop_differential`), not a single-pass verification. `design.md § Exported C ABI` fixes the *convention* + the `forget` *primitive*; this entry lands the round-trip once (a) or (b)'s gate clears.
...
**Open question (owner sign-off) — carried from the spike:** is the producer direction v1 or v1.x? The consume side is v1 (self-hosting / TLS need it); the producer direction is the *adoption* lever, which may not be a launch gate. Slices 2/3/3½/5 landed 2026-07-08 (the capability now exists and is proven); if adoption proof-points aren't a v1 *gate*, the remaining Slice 4 is v1.x. Flagged; not actioned.
