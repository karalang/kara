...
- **CI matrix — 4-target v1 hardening: all four legs landed 2026-07-09.** Stage 1 (macOS arm64, `codegen-e2e-macos`) confirmed the codegen + repr(C) ABI on Apple silicon and immediately paid for itself by catching a real arm64 `#[repr(C)]`-struct-by-value ABI bug (`B-2026-07-09-2`, **fully fixed** — ≤16 B register coercion + >16 B indirect/`sret` on arm64, and the paired >16 B x86-64 `byval`/`sret` hole it surfaced, all params + returns, hardware-confirmed). Stage 2 (`codegen-e2e-linux-arm64`, native `ubuntu-24.04-arm`) is the second arm64 execution leg (Linux AAPCS64, catches any Apple-vs-Linux divergence). Stage 3 (`codegen-e2e-macos-x86_64`, Intel `macos-13`) was **dropped**: it only covered the intersection of two already-tested things — x86-64 codegen+execution (`codegen-e2e`, ELF/SysV) and the macOS producer link path (`codegen-e2e-macos`, Mach-O) — so it added no unique coverage, and the scarce Intel `macos-13` runners left it queued 1h+ without allocating. **Stage 4 (`codegen-e2e-windows`, native `windows-latest` + LLVM 18 via Chocolatey) landed 2026-07-09** — Windows-native codegen + producer-mode E2E (library `.lib`/`.dll` artifacts + repr(C) enums + struct-by-value round-trips), paired with the Microsoft x64 aggregate ABI classifier fix (`B-2026-07-09-8`: 1/2/4/8-byte structs coerce to `iN`, else plain-ptr indirect / `sret` — no `byval`, since the caller allocates the copy). The Windows classifier is *also* verified on the Linux `codegen-e2e` runner via a `KARAC_FORCE_TARGET_ARCH=windows_x86_64` signature-match step (same trick used for arm64), so a coercion regression trips *before* the flakier Windows leg even runs. The matrix is now **4 targets**: Linux x86-64, Linux arm64, macOS arm64, Windows x86-64 — plus the pre-existing `windows-lint` clippy job.

**Owner decision (2026-07-08): producer direction is v1.** The flagship adoption pitch; the capability is shipped and proven (Slices 2/3/3½/5 + `forget`), and the remaining polish is bounded. **v1 scope line:** the previous release-blocking item — the CI matrix — landed as 4 targets on 2026-07-09 (Stage 4 Windows added alongside the `B-2026-07-09-8` Microsoft x64 aggregate ABI classifier fix); Windows library artifacts and Rust-host `std`-collision smoothing already shipped 2026-07-08. **Already shipped** (originally on the v1 or v1.x list): the core producer capability, project-mode `[lib]` table, the Path-A round-trip (`B-2026-07-08-4` closed), Path-B auto-boxing + nesting (pulled in from v1.x once the box-to-pointer design made it locally ASAN-verifiable), the book chapter + A/B snippet, and the category-specific rejection diagnostics. Deeper auto-boxing of `enum` / user-struct / >1-level nesting is resolved as won't-fix-by-design; `#[repr(C)]` tagged-union enums are a separately-scoped language feature.
