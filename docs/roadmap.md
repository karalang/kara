...
- **Remaining v1 hardening:** 5-target CI matrix — **stages 1–3 landed 2026-07-09.** Stage 1 (macOS arm64, `codegen-e2e-macos`) confirmed the codegen + repr(C) ABI on Apple silicon and immediately paid for itself by catching a real arm64 `#[repr(C)]`-struct-by-value ABI bug (`B-2026-07-09-2`, now **fixed** — params + returns coerced per AAPCS, hardware-confirmed). Stage 2 (`codegen-e2e-linux-arm64`, native `ubuntu-24.04-arm`) is the second arm64 execution leg (Linux AAPCS64, catches any Apple-vs-Linux divergence); Stage 3 (`codegen-e2e-macos-x86_64`, Intel `macos-13`) covers the macOS producer link path under x86-64 SysV + Mach-O. Each new leg runs the codegen E2E oracle + the producer-mode E2E (library artifacts + enums + structs). Stage 4 (Windows LLVM — the deferred `windows-lint` yak-shave, needs LLVM 18 on the Windows runner to cover the cfg(windows) library-link driver arms) is the remaining open leg.
