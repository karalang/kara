...
- **Remaining v1 hardening:** 5-target CI matrix — **stage 1 (macOS arm64, `codegen-e2e-macos`) landed 2026-07-09**, confirming the codegen + repr(C)-enum ABI on Apple silicon. It immediately paid for itself: it caught a real, pre-existing arm64 ABI bug in the `#[repr(C)]` **struct**-by-value-float path (`B-2026-07-09-2`, `stats_mean(Stats{f64,i64})` returns 0.00 not 7.50 on arm64) — that one test is quarantined from the arm64 leg with a bug ref while it's fixed. Stages 2 (Linux-arm64), 3 (macOS-x86_64), 4 (Windows LLVM) still open.
