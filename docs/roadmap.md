...
- **Ownership handoff across the boundary (Slice 4) — COMPLETE: `forget` + Path-A round-trip + Path-B auto-boxing all shipped.**
  ...
  - [x] **Auto-boxing / auto-destructor sugar — Path B.** The compiler auto-boxes a non-transparent aggregate return (`Vec[scalar]` / `String`) from a `pub extern "C" fn` and auto-emits its destructor — zero boilerplate. Kāra returns a `{data,len,cap}` value in registers (mismatches the SysV struct-return ABI), so the export heap-boxes it and returns an opaque **pointer** (a scalar return, C-compatible); the C side reads the fields transparently through the emitted struct and frees via `karac_free_<name>` (buffer + box). `codegen` (`current_fn_boxes_return` / `box_return_value` / `emit_export_destructors`) + `cheader` (`boxed_return_of` → struct typedef + `Struct*` return + destructor proto). An internal Kāra call to a boxed export is rejected (its LLVM return is `ptr`, not a Vec) rather than miscompiled. **Verified: Vec[i64] + String round-trip from a C host clean under ASAN/LeakSanitizer; drop_differential 0 divergences; memory_sanitizer 557 pass; codegen + cli green.** Non-scalar `Vec` elements (per-element drop) + `enum` returns stay opaque-`KaraHandle` — a follow-on within Path B.
...
**Owner decision (2026-07-08): producer direction is v1.** The flagship adoption pitch; the capability is shipped and proven (Slices 2/3/3½/5 + `forget`), and the remaining polish is bounded. **v1 scope line:** release-blocking = the shipped core + Windows library artifacts + 5-target CI matrix + project-mode `[lib]` table + Rust-host `std`-collision smoothing + the low-risk **Path-A round-trip** (raw-pointer methods `.offset`/`.read`/`.write`, `B-2026-07-08-4`). **v1.x (additive, non-blocking):** the auto-boxing / auto-destructor sugar (Path B) — its soundness gate is the CI-scale ASAN/LSan `drop_differential` corpus, so it must not gate the release; it lands additively once that gate is stood up. Book chapter + A/B verification also v1.x.
