...
- [ ] Task parking: Network I/O tasks park without blocking threads, resume on completion via the event loop. **Populating `KaracWaitTarget` with real `PeerTask`/`IoHandle` variants here (today it ships single-variant `None`, `runtime/src/lib.rs:316`) unblocks `KARAC_DEADLOCK_CHECK`** — see [`implementation_checklist/phase-7-codegen.md` § `KARAC_DEADLOCK_CHECK`](implementation_checklist/phase-7-codegen.md), blocked 2026-06-05 pending this. A "all workers asleep" deadlock is neither reachable (no real blocking primitive parks indefinitely in v1) nor distinguishable from idle until this lands.
...
- [ ] **Native library-artifact build mode (Slice 2).** `karac build --crate-type staticlib` (→ `.a`) and `--cdylib` (→ `.so`/`.dylib`), routing the [`design.md § Exported C ABI`](design.md#exported-c-abi) surface (`pub extern "C" fn` + `#[unsafe(no_mangle)]`) through the existing native link path with external linkage. Reuse `src/codegen/driver.rs` runtime-archive location logic. Explicit `-o` / `dist/` output — must **not** clobber a stray executable in CWD. macOS `cdylib` sets `-install_name @rpath/lib<name>.dylib`. Optional `[lib]` manifest table (artifact name + default kind; **not** the export list). **The produced artifact is not self-contained** — it links `libkarac_runtime.a`; verify a consumer links with **no karac toolchain present**.
- [ ] **C-header emitter (Slice 3).** Emit `lib<name>.h` (the cbindgen analogue; `--header` / `--no-header` override) for the exported surface, scoped to the Slice-1 type mapping — primitives + `#[repr(C)]` structs transparent, everything else opaque handle typedefs with accessor/destructor exports. Header carries include guard, `<stdint.h>`/`<stddef.h>`, `extern "C"` C++ guard, the two `karac_runtime_init`/`_shutdown` lifecycle prototypes, opaque typedefs, `#[repr(C)]` struct defs, and one `@effects`-annotated prototype per exported fn.
- [ ] **Ownership handoff across the boundary (Slice 4) — sequence AFTER ownership-mechanization's slice 2.** Build `forget` (roadmap L516, *suppress destructor; reserved for FFI handoff*) and specify who frees what: Kāra boxes a `Vec`/`String`, C returns it via a `karac_free_*` / `Drop` export rather than `free()`. **Hard dependency**: the export boundary is a move *out* of Kāra's ownership universe; co-design with [`spikes/ownership-model-mechanization.md`](spikes/ownership-model-mechanization.md) slice 2 or the two specs diverge. `design.md § Exported C ABI` fixes the *convention* (opaque + Kāra-owned destructor); this entry lands the *mechanism*.
- [ ] **Producer-side effect contract at the boundary (Slice 3½).** Extend `verify_extern_export_panics` from the `extern "C-unwind"` case to the full exported surface: reject a plain `extern "C"` export whose body's effect set contains `panics` unless declared abort-on-panic; reject an export whose effect set contains `suspends` (`E_EXPORT_SUSPENDS_UNSUPPORTED`) — v1 export boundary is synchronous-only. Producer-side effects are **KNOWN, not trust-not-verify** — the header states them precisely; do **not** copy the extern-*import* `{blocks}` default onto exports.
- [ ] **Proof-point demo/kata (Slice 5).** One hot parallel kernel written in Kāra, built as `.a` + `.h`, linked into an existing C **and** Rust host that keeps everything else. This is the artifact the adoption pitch actually sells; without it the capability is unproven. Becomes a permanent A/B-verified example ([`examples/`](../examples/), book-snippets-style A/B verify).

**Open question (owner sign-off) — carried from the spike:** is the producer direction v1 or v1.x? The consume side is v1 (self-hosting / TLS need it); the producer direction is the *adoption* lever, which may not be a launch gate. If adoption proof-points aren't a v1 gate, Slices 2–5 are v1.x. Flagged; not actioned.
...
- [ ] **`bench/compile_speed/` directory + curated corpus + synthetic stress program.** Three corpus members:
...
- [ ] **`bench/README.md`** — canonical bench-setup instruction covering hyperfine discipline (warmup/runs for short vs long workloads), rusage discipline (single-sample memory), artifact-deletion-for-cold protocol, output format. Adds **compile-elapsed time** as a tracked measurement alongside the existing compile-memory measurement (literally one missing block in the existing kata `bench.sh`, swapping `/usr/bin/time -l` for `hyperfine --warmup 1 --runs 10`). Mirrored as [`kara-katas/BENCH.md`](../../kara-katas/BENCH.md) so kata authors have a template.
- [ ] **Baseline: `rustc -O` (always in CI; the peer comparison on the same architectural surface — LLVM, monomorphization, ownership).** `clang -O2` measured opportunistically where C source exists (delta isolates karac-specific frontend cost from LLVM-backend cost); not a CI gate, no fresh-C-translation obligation. `go build` measured at launch-time only (published with explicit caveat: Go optimizes for compile speed by design; karac, like Rust, optimizes for runtime perf with proportional compile cost).
- [ ] **Cold only at v1.** Incremental compile-speed deferred until reactive query model lands (post-self-hosting).

#### CI workflow

- [ ] **Threshold: 30% initial, ≤5% long-term target.** Generous at start — during active development, legitimate changes shift compile time by a few % easily, and a tight gate would flake without catching the regressions that matter. The 30% gate catches order-of-magnitude blowups (Maranget shape) without false-positives on routine work. Tighten as data accumulates: corpus + 10–20 baseline runs reveals noise floor + steady-state karac-vs-rustc ratio, which together set the next threshold step. Doc commits to the trajectory (generous → tight); CI carries the current value.
- [ ] **PR-trigger workflow + main-merge baseline update.** PR runs the corpus, parses hyperfine output to JSON, compares against `bench/compile_speed/baseline.json` (committed to repo), posts PR comment with verdict + per-benchmark ratio (karac vs rustc) + per-benchmark delta vs baseline. Fails the job if any benchmark exceeds 30% over baseline. Separate main-merge workflow re-measures and updates `baseline.json` as a follow-up commit/PR. Stock GH Actions cache; no benchmark-result caching at v1.
- [ ] **`bench/bench.sh` top-level aggregator** — thin shell wrapper invoking each track's own bench script (`hash_quality`, `hot_swap_cost`, `indirection_cost`, `compile_speed`). One command runs everything; each track stays independently runnable.
...
- [ ] **Allocation reduction on hot String/byte paths** — *now the #1 real-world lever (was #2 at 38%; promoted after the string-match dispatch lever shipped — `malloc`/`free` is now the top self-time leaf, Rust gap 2.74×).* Two patterns dominate: (a) `substring` returns an **owned `String` copy** where a borrow/slice would do — the [`project_lexer_string_scan_shape`] zero-copy lesson, observed inside the lexer's own classify-only reads; (b) `Lexer.new` rebuilds the whole input into a `bytes: Vec[u8]` byte-by-byte (`for b in src.bytes()`). **Resolved into three concrete, separately-owned moves (2026-06-12):**
  - **SSO (small-string optimization) — the general, corpus-wide, compiler-owned lever. SCOPED as a staged campaign: [`spikes/small-string-optimization.md`](spikes/small-string-optimization.md).** Inline short strings in the `{ptr,len,cap}` struct → no `malloc` for the short-lexeme common case across *every* Kāra program. Central constraint: String shares `vec_struct_type` with `Vec`, so encode SSO via an in-struct tag (don't split the type). Highest blast radius in the String subsystem → deliberately staged for a fresh dedicated session (layout → tag-aware accessors → inline construction → runtime/FFI decode); full cold-start handoff + verification matrix in the spike. **This is the "close the gap / go further" lever.**
