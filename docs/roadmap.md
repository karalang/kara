...
| **M3 (parity)** | cross-platform parity | ⬜ open — **Windows IOCP** is the remaining socket-parity gate (macOS kqueue closed: functionally validated, scale Linux-only by design; Linux file-I/O io_uring is separate P1 work — phase-8, not a parity item); **the remaining v1-launch gate** (2026-06-07 decision: parity gates launch) |
...
- [ ] **String-literal `match` / `==` dispatch lowering** — *#1 real-world lever (46% of self-hosted-lexer self-time, 2026-06-12).* A `match` on string literals (e.g. `keyword_or_ident`'s ~90 `"kw" => Token` arms) and `==`-against-literal chains currently lower to a **sequential cascade of `memcmp`** — every identifier token walks up to ~90 string compares. Lower it to something sublinear: length-bucket + first-byte `switch`, a jump table, or a perfect-hash / trie over the literal set (the Rust lexer's equivalent `match` is far cheaper because LLVM builds a decision tree it can't build through Kāra's current lowering). General-purpose — every real program does string `match`/`==` dispatch (keyword tables, command/route dispatch, config/protocol parsing), so this compounds across the whole corpus, not one kernel. Single biggest lever found.
- [ ] **Allocation reduction on hot String/byte paths** — *#2 real-world lever (38% of self-hosted-lexer self-time, 2026-06-12).* Two patterns dominated: (a) `substring` returns an **owned `String` copy** where a borrow/slice would do — the [`project_lexer_string_scan_shape`] zero-copy-slice lesson, now observed *inside* the lexer's own classify-only reads; (b) `Lexer.new` rebuilds the whole input into a `bytes: Vec[u8]` byte-by-byte (`for b in src.bytes()`), a second full copy. Levers: slice-not-copy on reads that only classify/compare, `Vec`/`String` pre-sizing to kill growth-realloc, small-string optimization for the short-token common case. Pairs with — but is independent of — the dispatch lever above. (Related defect: `B-2026-06-12-10`, a suspected per-iteration leak in the same paths — verify under LSan.)
