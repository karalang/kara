...
- [~] **HTTP/2 — multiplexed streams + flow control (P1). SERVER SIDE SHIPPED 2026-06-03** (reconciled 2026-08-02 — the checkbox here was stale; the full shipped record is [`implementation_checklist/phase-8-stdlib-floor.md`](implementation_checklist/phase-8-stdlib-floor.md) line 172). The three `karac_runtime_serve_*` loops negotiate per-connection via hyper-util's `auto::Builder` — ALPN `h2` over TLS, h2c prior-knowledge over plain TCP, HTTP/1.1 fallback — with multiplexing/HPACK/flow control carried by hyper's `h2`, E2E-tested with real multiplexing h2 clients. Residuals keeping this `[~]`: **(a) client-side HTTP/2** — `Client.get`/`post` is ureq-backed HTTP/1.1-only; the h2 client is the actual prerequisite gRPC needs (`deferred.md § gRPC`); **(b) fully-async handler ABI** — sibling streams on one connection serialize while a handler runs (cross-connection parallelism preserved); named in the shipped entry as a separate, larger slice. Original scope line for reference: required for gRPC; sequenced after HTTP/1.1; HPACK, server push (default-off), `Server-Sent Events` interop.
