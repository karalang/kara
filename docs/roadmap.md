...
- **Ownership handoff across the boundary (Slice 4) — `forget` shipped; round-trip blocked on two foundations.**
  ...
  - [ ] **Full allocate→use→free round-trip — Path A (v1, release-blocking).** Implement the raw-pointer instance methods `.offset`/`.read`/`.write` (spec'd but unimplemented in codegen, `B-2026-07-08-4`; also `p[i]=x` index-store). With these + `forget` + `malloc`/`free` FFI, the sound manual round-trip is expressible — no per-type drop synthesis, no ASAN gate. This is the v1 completion of the handoff.
  - [ ] **Auto-boxing / auto-destructor sugar — Path B (v1.x, additive).** Compiler auto-boxes a non-transparent return into an opaque handle + auto-emits `karac_free_<T>`. Invasive return-ABI surgery + a per-type drop-glue synthesizer; **soundness gate = the CI-scale ASAN/LSan `drop_differential` corpus**, so it must NOT gate the v1 release — lands additively once that gate is stood up. `design.md § Exported C ABI` fixes the *convention* + the `forget` *primitive*.
