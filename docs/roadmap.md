...
**Owner decision (2026-07-08): producer direction is v1.** The flagship adoption pitch; the capability is shipped and proven (Slices 2/3/3½/5 + `forget`), and the remaining polish is bounded. **v1 scope line:** release-blocking hardening still open = 5-target CI matrix (Windows library artifacts + Rust-host `std`-collision smoothing shipped 2026-07-08). **Already shipped** (originally on the v1 or v1.x list): the core producer capability, project-mode `[lib]` table, the Path-A round-trip (`B-2026-07-08-4` closed), and Path-B auto-boxing + nesting — the last pulled in from v1.x once the box-to-pointer design made it locally ASAN-verifiable. Book chapter + A/B verification remain v1.x.
