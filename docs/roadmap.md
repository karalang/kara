...
- **Remaining v1 hardening:** CI matrix — **stages 1 & 2 landed 2026-07-09; stage 3 dropped as redundant.** Stage 1 (macOS arm64, `codegen-e2e-macos`) confirmed the codegen + repr(C) ABI on Apple silicon and immediately paid for itself by catching a real arm64 `#[repr(C)]`-struct-by-value ABI bug (`B-2026-07-09-2`, now **fully fixed** — ≤16 B register coercion + >16 B indirect/`sret` on arm64, and the paired >16 B x86-64 `byval`/`sret` hole it surfaced, all params + returns, hardware-confirmed). Stage 2 (`codegen-e2e-linux-arm64`, native `ubuntu-24.04-arm`) is the second arm64 execution leg (Linux AAPCS64, catches any Apple-vs-Linux divergence). The former Stage 3 (`codegen-e2e-macos-x86_64`, Intel `macos-13`) was **dropped 2026-07-09**: it only covered the intersection of two already-tested things — x86-64 codegen+execution (`codegen-e2e`, ELF/SysV) and the macOS producer link path (`codegen-e2e-macos`, Mach-O) — so it added no unique coverage, and the scarce Intel `macos-13` runners left it queued 1h+ without allocating. Each remaining leg runs the codegen E2E oracle + the producer-mode E2E (library artifacts + enums + structs). Stage 4 (Windows LLVM — the deferred `windows-lint` yak-shave, needs LLVM 18 on the Windows runner to cover the cfg(windows) library-link driver arms; the Windows x64 struct-by-value ABI itself is tracked separately as `B-2026-07-09-8`) is the remaining open leg. The matrix is now **4 targets**: Linux x86-64, Linux arm64, macOS arm64, + the Windows build/clippy jobs.

**Owner decision (2026-07-08): producer direction is v1.** The flagship adoption pitch; the capability is shipped and proven (Slices 2/3/3½/5 + `forget`), and the remaining polish is bounded. **v1 scope line:** release-blocking hardening still open = 5-target CI matrix (Windows library artifacts + Rust-host `std`-collision smoothing shipped 2026-07-08). **Already shipped** (originally on the v1 or v1.x list): the core producer capability, project-mode `[lib]` table, the Path-A round-trip (`B-2026-07-08-4` closed), Path-B auto-boxing + nesting (pulled in from v1.x once the box-to-pointer design made it locally ASAN-verifiable), the book chapter + A/B snippet, and the category-specific rejection diagnostics. Deeper auto-boxing of `enum` / user-struct / >1-level nesting is resolved as won't-fix-by-design; `#[repr(C)]` tagged-union enums are a separately-scoped language feature.
