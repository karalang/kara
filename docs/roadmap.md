...
- [ ] `StringSlice` — borrowed view into a `String` (pointer + offset + length); zero-copy parsing/splitting
  - The **core view type is LANDED and works on both backends**: `s.slice(a, b)`, `.find`, `.len`, `.to_string`, source-pinned `-> StringSlice` returns from a `ref` parameter, and a view held in an `Option`, a tuple, a struct field or a match binding (the last fixed by B-2026-08-26-38). A hand-rolled lazy `SplitIter` yielding `Option[StringSlice]` therefore compiles today and measured **9.0× faster / 5.2× smaller peak RSS** than `s.split(",")` on a 9.6M-token workload.
  - What remains is the **API shape**, which is what B-2026-08-26-13 is about: the stdlib offers no borrowing tokenizer, so reaching that 9× means every caller hand-rolls ~12 lines. design.md § StringSlice pins the intended fix as `split` itself yielding `Vec[StringSlice]` — explicitly **v2**, because it needs *a borrow flowing through a collection*, which the type system cannot express (`Vec[StringSlice]` is rejected at typecheck today).
  - **Open decision, and the reason this is not just "implement v2":** a cursor/iterator form needs NO collection-of-borrows and is expressible now, so it could ship well before the v2 lifetime work. Shipping it means choosing a second spelling alongside the eventual `split`-yields-views, or committing to the iterator as the borrowing form and leaving `split` owned forever. `String.split_whitespace` and `String.lines` carry the same shape and should move together whichever way it goes.
...
- [x] **HTTP/2 — multiplexed streams + flow control (P1). SERVER SIDE SHIPPED 2026-06-03; CLIENT SIDE SHIPPED 2026-08-02; COMPLETE.** (Server-side record: [`implementation_checklist/phase-8-stdlib-floor.md`](implementation_checklist/phase-8-stdlib-floor.md) line 172 — the checkbox here was stale until the 2026-08-02 reconciliation.) **Server:** the three `karac_runtime_serve_*` loops negotiate per-connection via hyper-util's `auto::Builder` — ALPN `h2` over TLS, h2c prior-knowledge over plain TCP, HTTP/1.1 fallback — with multiplexing/HPACK/flow control carried by hyper's `h2`, E2E-tested with real multiplexing h2 clients. **Client:** `Client.get`/`post`/`request(...).send()` now ride a hyper `client::conn` transport (replacing the ureq HTTP/1.1-only one) — HTTP/2 via ALPN over TLS with HTTP/1.1 fallback, plain HTTP/1.1 over `http://`; full webpki verification against bundled Mozilla roots with `KARAC_HTTP_EXTRA_CA=<pem>` for private CAs; redirect-following (5 hops, 301/302/303 POST→GET demotion) and the `status >= 400 → Err` mapping preserve the previous transport's contract (which the interpreter's ureq path still exhibits — run-vs-build parity observes status/body/header values). E2E: h2-negotiated exchange asserted via a version-echoing TLS origin, h1-fallback pin, redirect pin (`tests/http_server.rs`). The h2 *client* was the actual gRPC prerequisite (`deferred.md § gRPC`) — that gate input is now satisfied. The last residual — the claimed sibling-stream serialization needing a "fully-async handler ABI" — closed 2026-08-02 by EMPIRICAL CORRECTION: streams on one connection already run concurrently (hyper's h2 server spawns per-stream service tasks on the `TokioExecutor`; `block_in_place` pins only the one stream's worker), pinned by `tests/http_server.rs::test_http2_sibling_streams_run_concurrently` (two 500 ms handlers, one connection — sleeps proven run AND overlapped). What survives of it is an efficiency note, not a gap: each in-flight handler occupies an OS thread; a coroutine handler ABI (park in-flight handlers threadlessly) is future perf work, tracked in the phase-8 entry's corrected paragraph. Original scope line for reference: required for gRPC; sequenced after HTTP/1.1; HPACK, server push (default-off), `Server-Sent Events` interop.
...
- [x] **`karac debug <crash.json>`** — one-shot crash-report renderer. *(Slice 3, 2026-07-23: `src/crash_report.rs` + the `debug` subcommand. Reads JSON — file path or `-` for stdin — and renders human-readable: panic site + kind + message, the effect set via the shared `effect_render` compact form, the logical stack, parallel context (which `par` branch panicked + worker, siblings still running, siblings cancelled at their effect boundary), provider stack, RC-fallback annotations, and a build-metadata footer; `--output=json` re-emits the structured JSON pretty-printed. `std.panic` doesn't emit yet, so this consumer pins down the concrete wire schema for design.md § 4's eight fields — the fixture `examples/crash/user_dashboard_panic.json` is the executable spec — and parses leniently (every field optional, unknown keys ignored) so the format can grow additively. `caused_by` / `concurrent_with` / `tracing` / `gpu_marker` optionals handled. Deferred within this item: reading the referenced source file to show the panic *line* with a caret (the crash JSON travels without its sources), and the ASCII parent-task graph glyphs — the current per-frame stack list carries the same information.)*
- [ ] **`karac inspect <pid>`** (Linux + macOS at v1) — attaches to a running process via `ptrace` (Linux) or `task_for_pid` (macOS); reads runtime metadata via the per-worker cooperation hook from Track-5-adjacent Gap 2 work (same surface, reused); dumps `list_tasks()` / `list_par_blocks()` output without requiring code changes. Equivalent to Go's `go tool stack`. `--once` (default) for one-shot; `--watch` for periodic re-dump (high incident-response value). **Windows `karac inspect` deferred to v1.x** (different debug APIs: `DebugActiveProcess` / `ReadProcessMemory`). **Blocked — feasibility + design spike at [`docs/spikes/karac-inspect-feasibility.md`](spikes/karac-inspect-feasibility.md) (2026-07-23):** the gating prerequisite is **Phase 6.3 suspension tracking** (real `KaracWaitTarget` variants + parked-task registration in `event_loop.rs`; today `list_tasks()` is empty *by design* — v1's blocking model tracks no suspension, so on the done-when's target (a parked HTTP server) `inspect` would have nothing to report). The spike records the sound design for when unblocked (a cooperative **responder-thread + signal + dump-file** transport rather than ptrace — layout-safe and async-signal-safe; the cooperation-hook design; the snapshot schema + an `effect_render`-backed renderer) and the 5-step sequencing. The transport/renderer/CLI (steps 3–5) are bounded; step 1 is deep event-loop work that must be scheduled with real network-load testing, not under the Track 6 polish umbrella.
...
- [x] **Rendering logic lives in a shared crate/module used by `karac` binary and LSP server.** *(Slice 1–2, 2026-07-23: the logic lives in `src/effect_render.rs` — a module in the compiler lib crate the `lsp` member already depends on, so it is shareable without a separate crate. Every independent copy of the verb→keyword map now routes through `effect_render::verb_keyword`: the REPL footer (slice 1), the LSP hover (`render_effect_set` → `render_compact`, now group-first `+`-joined and consistent with the doc generator, fixing a latent alphabetical/collision divergence), the wasm-safe `effect_graph` JSON emitter behind `karac query effects` + the browser studio, the C-header emitter, and the formatter. `catalog::effect_verb_kind_str` deliberately stays separate — its `UserDefined → "user_defined"` is a category label, not the keyword. Slice 3 added the crash-report entrypoints (`crash_report::render` for the human form; `karac debug --output=json` re-emits the structured JSON), reusing `render_compact` for every effect summary in the report. Slice 4 (2026-07-23) wired the LSP consumer: the `kara/renderCrashReport` custom request (`lsp/src/analysis.rs::render_crash_report`) calls the same `crash_report` model + renderer the compiler binary uses, so a "Kāra: Render Crash Report" editor command gets byte-identical output to `karac debug`. Both consumers — CLI binary and LSP server — now share the one rendering module, closing this box.)* Original intent: Extends the existing structured-diagnostic infrastructure (same machinery as compile-time error rendering: source-span highlighting, color/no-color, terminal width, ANSI). Don't fork; load-bearing primitives are shared between compile-time and runtime diagnostics. API exposes structured + rendered forms together (e.g., `render_crash_report(report, opts) -> String`, `crash_report_to_json(report) -> serde_json::Value`). LSP server calls the same functions and routes output into LSP-shaped responses. **Non-Rust LSP future**: via subprocess + JSON, not FFI; JSON contract is stable, Rust crate is an implementation detail.
...
- [x] **3am-operator runbook in the book** — *(Slice 6, 2026-07-23: `docs/book/src/ch20-debugging-crashes.md`, "The 3am Runbook: Debugging Crashes", new "Operating in Production" book section.)* Short and starter-shaped, not exhaustive. Contents: (a) entry point ("when paged with a crash JSON, run `karac debug <file>` first"); (b) three worked examples — a resource/index panic naming its blast radius, a `drop_during_unwind` with `caused_by` + RC-fallback annotation, and a `par_fail_fast_cancel` batch — each with **actual `karac debug` output** captured from committed fixtures (`examples/crash/*.json`); (c) a common-patterns table (effect set on the panic frame → where to look first); (d) escalation via `--output=json` for diffing / bug reports / AI agents. Grows from incident data once Kāra is in production.
...
- [ ] **GPU compute shaders — v1 ship gate (P1).** Compile `#[gpu]`-annotated functions to GPU kernels and wire `gpu.dispatch` to invoke them. Full design of the `#[gpu]` constraint, `GpuSafe` type bound, and `gpu.dispatch` effect semantics is already in `design.md § GPU Subset Constraints`. Pulled forward from Phase 10 to v1 on 2026-05-11 (v66 graduation). Multi-vendor coverage via the wgpu-primary path (below) satisfies the dogfooding requirement (project leader develops on macOS — Metal coverage at v1 is non-negotiable) and the systems-language-target-completeness requirement.
  ...
  **Implementation tasks:** — *granular slice breakdown lives in [`implementation_checklist/phase-10-targets.md`](implementation_checklist/phase-10-targets.md) "GPU compute shaders — slice breakdown"; the coarse boxes below mirror it.*
...
> - **Compiler-internal NOT yet built — built once, directly in Kāra.** `f16`/`bf16` *lowering* (keywords reserved, LLVM `half`/`bfloat` emission not), `Secret[T]` derive codegen, the entire **Embedded / Hardware Primitives** subsection (inline `asm`/`global_asm`, volatile intrinsics, `#[interrupt]`, linker control, `Atomic` codegen), the **Codegen Optimization (IR quality pass)**, and Phase 10's residual GPU codegen — *narrowed 2026-07-10: the wgpu/WGSL GPU backend was pulled out of this bucket by owner directive and built in Rust `karac` (shipped; see the Phase 10 GPU tracker), so the residual here is only the unbuilt CUDA/NVPTX path (CG-5)*. **This is the only bucket the pivot saves work on** — don't add these to the Rust `karac` after the pivot; build them straight into the self-hosted compiler.
>
...
**Codegen emits LLVM IR as TEXT — it needs no FFI bindings at all.** *(Approach REVISED 2026-07-12. This paragraph previously read "Codegen needs LLVM-C FFI bindings… a large chunk of in-phase work," specifying an `extern "C"` binding layer as the `inkwell` analogue. That is no longer the plan, and the work it budgeted for is not needed.)* The self-hosted codegen module is a pure `AST → IR-string` transform: it emits `.ll` text, which the shipped `karac_jit_runner` already executes — so **no inkwell and no LLVM-C bindings**. Proven by [`spikes/selfhost-backend-feasibility.md`](spikes/selfhost-backend-feasibility.md) and shipped through slice 64 in `selfhost/src/codegen.kara` (8.7k lines, zero `extern "C"`). The trade is the oracle: IR text is not character-reproducible (SSA numbering and block labels are construction-order artifacts), so the codegen gate is **behavioral** — program-output parity against the seed — rather than the byte-identical diff the front-end ports enjoy. The three `self-hosting-llvm-c-*` spikes that specified the binding layer are kept as history and marked SUPERSEDED.

**Per-stage state.** `[x]` = fully ported · `[~]` = partially ported, slices landed and passing their oracle · `[ ]` = not started (the marker vocabulary of [`implementation_checklist/`](implementation_checklist/), see [`deferred.md § Tracker status markers`](deferred.md#tracker-status-markers)). [`implementation_checklist/phase-12-self-hosting.md § Port sequencing`](implementation_checklist/phase-12-self-hosting.md) is canonical and carries the slice-by-slice detail; this list is its summary.

Every stage is gated by a **differential oracle** against the Rust seed (`tests/selfhost_*.rs`) — the port and the seed run the same corpus and their rendered output is diffed. So `[~]` here means "passing its oracle at the slice reached," not "drafted." The front-end oracles run under plain `cargo test` (CI's `test` job); `selfhost_codegen` is `--features llvm`-gated and deliberately excluded from CI's `codegen-e2e` job on runtime budget (~181 s), so it is a developer-run gate.

- [x] **Lexer in Kāra** — complete. Full token set (delimiters, maximal-munch operators, keyword table, all literal forms, f-string interpolation, c-strings, doc comments), `\u{…}`/`\0` escapes, multi-byte codepoint bodies, raw identifiers, reserved-form error recovery, and multi-line span/column tracking. Passes `tests/selfhost_lexer.rs` bit-for-bit; CI's `oracle-sync` job fails any `src/lexer.rs` change that doesn't update the oracle, so the port can't silently drift from the seed again.
- [~] **Parser in Kāra** — expressions through slice 3b, type expressions (3a), and items (3c-i…v + 3d-i: `use`/`const`/`type` alias, struct + enum, free functions, `trait` + `impl`, bare `[T, U]` generic params, attributes + doc comments). ~458 corpus entries across `tests/selfhost_parser{,_types,_items}.rs`. Still out of scope: trait bounds (`[T: Ord]`), variance / const / shape-variadic / effect generic params, and supertraits.
- [~] **Resolver in Kāra** — slices 1–2c: a single `fn` (generics, `self`, params, return type, body scopes), the declaration items (`struct`, `enum`, `type` alias, `const`) in fused collect-and-resolve form, and `trait` + `impl` scopes. `tests/selfhost_resolver.rs`.
- [~] **TypeChecker in Kāra** — through slice 21, ~224 corpus programs (`tests/selfhost_typechecker.rs`). Infers a coarse category per expression (NUM / BOOL / CHAR / STR / UNKNOWN, reproducing the seed's literal leniency exactly) over a real typing substrate: binding and parameter environment, operator result types, annotated `let`, call-return typing, struct-literal and field-access typing, method-call typing. Diagnostics ported so far: return-type mismatch, `ConditionNotBool` on `if` and `while`, `NonExhaustiveMatch`, enum-vs-other mismatch, `MissingField` / `ExtraField` / `UndefinedField`, argument-count, `InvalidUnaryOp` / `InvalidBinaryOp` (distinct-primitive comparison, logical operands, arithmetic and bitwise operands), `StringNotIndexable`, `NotCallable`, and `BranchTypeMismatch` for `if`/`else` and match arms.
- [ ] **EffectChecker in Kāra** — not started.
- [ ] **OwnershipChecker in Kāra** — not started.
- [~] **Codegen in Kāra** (textual LLVM IR emission — no FFI) — through slice 64, on a 211-program **behavioral** oracle (IR text isn't reproducible character-for-character — SSA numbering and block labels are construction-order artifacts — so the gate is program-output parity against the seed's `karac run`, not an IR diff). Covers integer arithmetic, `let`/assignment/shadowing, bools and comparisons, `if`/`else`, `while`, `as` casts, unary negation, bitwise and shift operators, indexed assignment, `Vec` push/pop and `String.push_str` (including field receivers), value-position `if`/`match` with typed result slots, struct destructuring, `shared enum` with the RC pointer representation, and `.clone()`.
- [ ] **CLI / driver in Kāra** — not started.
