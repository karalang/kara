...
| **Prism** | Web-convenient *and* fully-local image compute (Squoosh-class, zero upload); Kāra owns the pixel math; **multicore via threaded exports** | ✅ shipped | **live at [karac.dev/prism](https://karac.dev/prism)** (multicore there too — vendored coi-serviceworker supplies COOP/COEP on GitHub Pages); built at `kara-katas/apps/prism` — image-in (`read_src`) / canvas-out host fns; kernels: Lanczos-3 + bilinear resample (both **banded across the pool** via `instantiateThreaded`, 2.1–2.8× measured, byte-identical to sequential), crop/rotate/flip/adjust/grayscale; chaining working-image UI + undo + PNG/JPEG/WebP download; 16 exact-oracle node tests + real-Chrome CDP verification of THREE legs (sequential fallback, real-COOP/COEP threaded, coi-shim headerless→threaded). Drove kara fixes/features B-2026-07-20-10/-13/-14. Deferred: SIMD inner kernels, EXIF panel, native CLI leg | 3 |
| **Veil** | Privacy-first local redaction — the sensitive file never leaves the device; a no-install web app vs upload-first incumbents | ✅ shipped | **live at [karac.dev/veil](https://karac.dev/veil)**; built at `kara-katas/apps/veil` on Prism's spine — draw-a-box UI; kernels: pixelate (tile mean), rect-contained 3× box blur, solid bar (the destroys-pixels safe default, stated honestly in the UI); EXIF stripped on re-encode; exact-oracle node tests (incl. a hand-traced 3-pass blur) + real-Chrome CDP verification. Deferred: multi-box redact-all, `Regex` PII auto-suggest | 3 |
...
> move-suppression so the caller owns the buffers). Slice 4 extends the forward
> inference to the **borrow forms** `ref Vec[E]` / `mut ref Vec[E]` — the way
> real multi-buffer kernels (`grid`, `coll`, `next`) share helpers without moving
