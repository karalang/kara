...
> **Implementation status (2026-08-25): the surface above works on all four execution surfaces** — `karac build`, `KARAC_AUTO_PAR=0 karac build`, `karac run` (JIT) and `karac run --interp`, byte-identical. `B-2026-08-25-2` closed. It was INTERPRETER-ONLY between 2026-05-11 and that row: every pure-Kāra instance method on these types (`Arg.string().required()`, `parser.about(…)`) died in codegen with `no handler for method '<m>' on variable '<v>'`, so `karac check` passed and `karac build` failed while this section described the feature as shipped. Codegen compiles the module's method bodies only for programs that use its types (`program_uses_cli`); its struct/enum LAYOUTS are registered unconditionally, so naming one of these types costs nothing and cannot silently lower at the wrong width.
...
**Cross-reference:** P2 § `par for` — Data-Parallel Loop Syntax (superseded; records why the keyword is declined); `B-2026-07-29-29` (queryability, hard prerequisite); `B-2026-07-29-30` (the `#[par_unordered]` -> `#[par_order_free]` rename — the adjacent collect path); `design.md § 8876` (the enumeration this extends); `phase-7-codegen.md` § ILP/noalias (the miscompile-risk precedent and the harness pattern); `CLAUDE.md` § *no workarounds — fix the compiler* (the rule under which Prism's hand-rolled fan-out is a placeholder, not a design).
...
Three further reasons: the escape hatch **already exists** (`TaskGroup` is shipped, and its verbosity is appropriate for code whose safety could not be proven automatically); this entry's own text concedes `par for` is *"syntactic sugar with an independence check, not a new capability"*, so the check was always the feature; and keyword economy — `par {}`, `seq {}`, `par struct`, `par enum`, and `#[par_order_free]` already exist, so a fifth `par` spelling carrying no new capability is permanent surface area in every tutorial.
...
- ~~**(d, common) Map values that aren't Vec/String.** `Map[K, Map[J, V]]`, `Map[K, Holder]`. The `karac_map_free_with_drop_vec` helper specifically expects the value to follow the `{ptr, len, cap}` shape — anything else routes to plain `karac_map_free` (`val_is_vec = false`) and leaks the entire value. Slice δ requires generalizing the runtime helper to accept a per-value drop fn pointer (analogous to `slice::sort_by`'s comparator FFI). **CLOSED 2026-07-01 (owned-temp spike slice 3r).**~~ Exactly the prescribed fix: `karac_map_free_with_val_drop_fn(map, drop_key, val_drop_fn)` (+ the `clear` sibling) runs a synthesized `karac_drop_<V>(ptr)` on every live entry’s value blob in place; `map_val_drop_fn_for_type_expr` selects it for user struct/enum values, inner Map/Set values (the upgraded `emit_map_drop_fn` recurses — `Map[K, Map[J, Map[..]]]` drops its deepest strings, which also closes gap (a)’s `Map[K, Vec[Vec[T]]]` value leg), Option/Result inline payloads, and Vec-shaped values whose element owns heap (`Map[K, Vec[String]]`); shared V keeps the rc-dec walk, plain String/Vec[POD] values keep the flag fast path. The slice also fixed two Map-value correctness bugs the probes surfaced: `m.insert(k, <map binding>)` UAF (no Map/Set arm in the consume-site suppression walk — now a branch-safe null-store) and the wide-payload `Map.get` boxed-scrutinee double-free (box-only free for borrow-call scrutinees), plus the discarded boxed-Option temp leak (`m.remove(k);` / displaced overwrite values — `try_track_discarded_boxed_option`). Remaining sibling: B-2026-07-01-12 — moving a borrow-bound `Map.get` payload OUT of the match arm double-frees (pre-existing, crashes today on `Map[i64, String]`; needs a deep-clone at the arm-tail move).
- **(a, less common) Deeper-than-one-level nesting.** `Vec[Vec[Vec[T]]]`, `Map[K, Vec[Vec[T]]]`. The shipped one-level inline drop (`FreeVecBuffer` cleanup arm) frees the middle layer's data buffer but the per-element body is just `free(inner.data)` — no recursion into the middle layer's *element* drop. Innermost buffers leak. Slice δ requires per-elem-type drop-fn synthesis with a cache keyed on the element type, recursive on Vec/Map/Set/struct/enum. **PARTIALLY CLOSED 2026-06-30 (owned-temp spike slice 3n).** The `Vec[Vec[…]]` *element* leg is closed for element subtrees the recursive drop family fully frees: `Vec[Vec[String]]`, `Vec[Vec[Vec[T]]]`, `Vec[Vec[Map[..]]]`, `Vec[Vec[Set[..]]]`, `Vec[Vec[(..heap..)]]`. `vec_elem_agg_drop_for_type_expr` (runtime.rs) now returns the already-built strictly-recursive `emit_vec_drop_fn` (= the `emit_drop_fn_for_type_expr` family the architectural sketch below describes, built later and previously `#[allow(dead_code)]`) for a `Vec[heap-inner]` element, so it routes through the agg-drop cleanup branch and drops every level. Every consult site (let-binding, for-loop, fresh-temp, nested `synth_drop`) inherits the fix. Gated so a `Vec[Vec[scalar]]` element stays on the (exact) one-level fast path. **Extended 2026-06-30 (slice 3o):** the `Vec[Vec[<user struct / enum / shared>]]` element leg is now closed too — `emit_drop_fn_for_type_expr`'s named-type arm delegates to `vec_elem_agg_drop_for_type_expr` (frees value heap fields via `__karac_drop_struct_<S>`, drops enum payloads via `__karac_drop_<Enum>`, rc-decs shared elements), and `te_recursive_drop_fully_supported` admits `struct_types`/`enum_layouts`/`shared_types`. Choosing the Vec-element-aware drop (not plain `emit_struct_drop_synthesis`) is what makes it correct for a nested element with no `let` cleanup for its shared fields. **Extended again 2026-06-30 (slice 3p):** `Option[String]` / `Option[Vec[..]]` elements are closed too — the tag-guarded `karac_drop_Option_<payload>` (`emit_option_drop_fn`) frees the inline `Some` payload overlay, and the push family disarms a moved Option-binding source (`suppress_inline_option_payload_cleanup_for_moved_arg`). Slice 3p also fixed the inferred-`str`-spelling hole that silently kept UNANNOTATED cross-fn bindings (`let v = build()`) on the leaky fast path across 3n/3o/3p shapes (annotations spell `String`; `type_to_type_expr(Type::Str)` spells `str`). **Extended 2026-07-01 (slice 3q):** `Result[T,E]` elements closed too (tag-dispatching `karac_drop_Result_<ok>_<err>`; heapless sides emit no call), plus the loop-element consume-from-copy double-free fix (`for_loop_borrow_vars` marking + the borrow flag at the match/if-let/while-let/let-else bind sites). **Still open in (a):** non-inline (boxed/wide/handle) Option/Result payloads (the delegate returns None → one-level fast path retained); struct fields that are THEMSELVES structs-with-heap (a pre-existing `emit_struct_drop_synthesis` nested-struct-field limit, orthogonal to the Vec nesting); the `Map[K, Vec[Vec[T]]]` *value* leg (that's gap (d) — the Map value-drop helper, not the Vec element path).
...
**The pre-launch slice that IS worth taking now — ✅ LANDED 2026-08-02:** a worked consume-direction binding of a real C library as [`examples/interop-zlib/`](../examples/interop-zlib/) + binding guide in its README — zlib compress→decompress→CRC32 round-trip through an effect-annotated `unsafe extern "C" { }` block and `[link] libs = ["z"]`, verified byte-identical to a C reference implementation on three surfaces (default auto-par build, `KARAC_AUTO_PAR=0`, JIT run). It is binding #1 of the promotion-gate corpus, and it did the kata job on the way in — three ledger entries filed from authoring it: B-2026-08-02-4 (FFI lint suggests `allocates(Heap)` without the `effect resource Heap;` declaration; E0100 lacks the suggestion), B-2026-08-02-2 (no implicit `*mut T` → `*const T` weakening at call sites — every binding hits it; design fork recorded), B-2026-08-02-3 (interpreter refuses pointer FFI via raw panic + backtrace instead of a structured diagnostic; plus the incidental-JIT-symbol-resolution observation).
