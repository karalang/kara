...
- [ ] **`vec![...]` list-macro form unimplemented — parser rejects the `!`.** Surfaced by the docs/book audit (2026-06-13). `vec![1, 3, 5]` fails with `error[parse]: Expected Semicolon, found Bang`, yet `design.md:543` uses `let v = vec![todo()];` as a canonical inference example, so the form is spec-blessed. Today the only working vector-literal idiom is `Vec.new()` + `.push()` (see every `examples/*/src/*.kara`). **Book is ahead of compiler:** `docs/book/src/ch01b-two-surfaces.md` uses `vec![...]` 4× (incl. the headline binary-search demo) and is left spec-accurate by design-owner decision — that block will not compile until the macro lands. **Where to fix:** lexer/parser recognition of the `ident!` macro-call head + lowering `vec![a, b, c]` to the `Vec.new()`/push sequence (or a dedicated literal node). Cross-ref: the repeat-literal `[v; n]` / `Vec[v; n]` work at [`phase-4-interpreter.md`](phase-4-interpreter.md) line 11 (sibling literal form).
