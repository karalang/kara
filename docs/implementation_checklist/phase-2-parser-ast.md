...
- [x] **`vec![...]` list-macro form.** Closed 2026-06-19. Surfaced by the docs/book audit (2026-06-13): `vec![1, 3, 5]` failed with `error[parse]: Expected Semicolon, found Bang`, yet `design.md:543` uses `let v = vec![todo()];` as a canonical inference example, so the form is spec-blessed. **Fix (parser-only desugar):** `parse_identifier_expr` now recognizes the `vec` + `Bang` + `[` head (two-token lookahead via `peek_token_at(1)`) and routes the bracket body through a newly extracted `parse_prefix_collection_literal(name, start)` helper with `type_name = "Vec"` — so `vec![a, b, c]` produces the *same* `PrefixCollectionLiteral { type_name: "Vec", .. }` node as `Vec[a, b, c]`, and `vec![v; n]` the same `RepeatLiteral { type_name: Some("Vec"), .. }` as `Vec[v; n]`. No new AST node, no `!`-macro machinery: every downstream phase (resolver/typechecker/effect/ownership/interpreter/codegen) sees an ordinary `Vec` literal and needed zero changes. `vec!` is the only blessed list macro — any other `ident!` still falls through to the bare-identifier path and the `!` errors as before. The block-extraction is behavior-preserving for the existing `Vec[...]`/`Array[...]`/`Set[...]`/`Map[...]` call site (verified by the unchanged 707-test parser suite). **Tests:** 5 parser tests (`test_vec_macro_literal`, `_empty`, `_repeat_form`, `_nested`, `test_non_vec_bang_still_errors`) in `tests/parser.rs`; 1 interpreter E2E (`test_vec_macro_literal_end_to_end`, covers both list + `vec![0; 4]` repeat forms) in `tests/interpreter.rs`; 1 codegen E2E (`test_e2e_vec_macro_literal`, list form) in `tests/codegen.rs`. **Note surfaced during codegen verification:** `vec![v; n]` (like `Vec[v; n]`) still hits the pre-existing `codegen: Vec[v; n] repeat literal not yet supported` gap — that repeat-literal codegen lowering is the separate sibling item at [`phase-4-interpreter.md`](phase-4-interpreter.md) line 11 and is unchanged by this work (the interpreter handles the repeat form today; codegen does not).

- [x] **Top-level `pub const X: T = lit;` not visible from function bodies (`Undefined variable`).** Closed 2026-05-08. **Root cause: hypothesis (c).** Codegen had zero `Item::ConstDecl` handling — interpreter handles consts at `interpreter.rs:2057` (matched arm in `eval_program` defines the name in `self.env`) so single-file `karac run` always worked, but `karac build` walked through `compile_program` without a const-collection pass and a bare `WORK` reference in a function body parses as `ExprKind::Identifier("WORK")` which fell through to `load_variable` (`codegen.rs:14009`) and emitted "Undefined variable 'WORK'". Hypotheses (a) and (b) were not the issue — the resolver registers consts module-wide and the typechecker has both `env_add_const` and `check_const_decl`. **Fix:** added `consts: HashMap<String, Expr>` to `Codegen` state, populated in `compile_program` ahead of function compilation, and inserted a lookup in the `ExprKind::Identifier` arm before the `load_variable` fallback that re-compiles the stored value expression at the use site (LLVM folds the resulting constant arithmetic). Re-compile-at-use-site naturally handles transitive const references (`pub const SCALED: i64 = BASE + BASE;`) since the inner `Identifier` lookup hits the same path. Cycles are precluded upstream by the typechecker's const-evaluation pass. **Tests:** two new E2E tests in `tests/codegen.rs` — `test_pub_const_visible_in_fn_body` (the canonical repro, sum(0..WORK)) and `test_pub_const_references_other_const` (transitive). Slice 6 workaround removed: `examples/parallax_lite/src/workload.kara` now declares `pub const WORK: i64 = 50000000;` at the top of the module and the three drivers reference `WORK` directly (one source-of-truth for benchmark-sweep tuning). Parallax-lite test suite (`tests/parallax_lite.rs`) stays green.
