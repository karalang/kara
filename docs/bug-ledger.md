...
| B-2026-09-09-5 | 2026-09-09 | other | low | ONE TEST IN THE REQUIRED GATE SET IS STILL UNATTRIBUTED, AND THIS ROW'S ORIGINAL THREE ARE ALL EXPLAINED -- the ASAN pair by B-2026-09-09-4 (`fb05f38` re-floored a vacuity guard that a contended sweep had already breached at 87 against 90) and the `tests/interpreter.rs` member by B-2026-09-09-7's full disk, so the title this row carried was stale rather than open. What remains is `signalling_karac_run_does_not_orphan_the_jit_runner`, which failed once on 2026-09-10 and once on 2026-09-12 under heavy self-inflicted load and passed in isolation both times, with the assertion text filtered away on both occasions so neither failure can be pinned to a LINE. The zombie-counting hazard this row recorded as latent is now measured on the LIVE path and removed (`runner_alive` counted an exited-but-unreaped runner as a surviving orphan; SIGKILL-specific, load-amplified, 1.03s quiet and 1.86s under 8 hogs), and the row's leading hypothesis is corrected: it assumed the 15s orphan assert, but that assert sits against a 200ms watchdog poll, while the SAME test carries a 5s IR-file assert whose leg B-2026-09-10-10 measured racy at 2 of 4 full-gate runs against 0 of 2 isolated | — |
