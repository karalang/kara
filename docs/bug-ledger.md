...
| B-2026-09-03-17 | 2026-09-03 | resolver+interp+codegen | medium | A user-defined ASSOCIATED FUNCTION (no `self`) on a PRIMITIVE type passes `karac check` clean and then fails on EVERY executor. MINIMAL (3 lines): `trait Zero { fn zero() -> Self; } impl Zero for i64 { fn zero() -> i64 { return 0; } } fn main() { let x: i64 = i64.zero(); println(x.to_string()); }` -- `karac check --output=json` reports 0 diagnostics; `karac run --interp` errors `internal: name 'i64' resolved but has no binding at run time. This is a compiler bug (the resolver should have rejected or bound it)`; `karac run` (JIT) and `karac build` both error `codegen: no handler for method 'zero' on variable 'i64' (method dispatch fell through; this is a codegen bug)`. So all three backends reject what the front end accepts, and two of the three messages declare themselves compiler bugs. NOT a run-vs-build divergence -- run and build agree, and both disagree with `check`. SCOPED THREE WAYS, and the discriminator is `self`, not primitives: (a) an INSTANCE method via a trait impl on a primitive WORKS on both backends -- `trait Dbl { fn dbl(ref self) -> i64; } impl Dbl for i64 { fn dbl(ref self) -> i64 { self * 2 } }` then `x.dbl()` prints 42 under interp and JIT alike -- so `impl <trait> for <primitive>` is a supported feature generally and only the no-self form is unreachable; (b) the same associated function reached through a GENERIC TYPE PARAMETER WORKS -- `fn make[T: Zero]() -> T { return T.zero(); }` called as `let x: i64 = make();` prints 0 on both backends, correctly monomorphising to the i64 impl, which proves the impl is reachable and dispatchable and that only the syntactic form `i64.zero()` has no resolution; (c) it is not trait-specific -- an INHERENT `impl i64 { fn two() -> i64 { return 2; } }` with `i64.two()` fails identically on both backends. UNIFORM ACROSS PRIMITIVES: i32, i64, u8, bool and f64 all give 0 check diagnostics and the same codegen fall-through. The same associated-function form on a USER STRUCT works (`impl Zero for P { fn zero() -> P { P { x: 7 } } }` then `P.zero()` prints 7), and the built-in type-name receiver works too (`i64.MAX` prints 9223372036854775807), so a primitive type name IS resolvable as a receiver -- just not for a user-defined associated function. ROOT (from the interpreter's own message): the receiver `i64` is being resolved as a NAME/variable rather than as a type, so lowering looks for a binding that never existed; codegen's `compile_method_call` then sees a receiver it calls a `variable` named `i64` and falls through. RELATED AND COMPLEMENTARY: B-2026-07-22-10 (fixed) covered the UNKNOWN-name half of exactly this surface -- `i64.max_value()`, a Rust-ism -- and its fix now yields a clean `E0236 no associated function 'max_value' on type 'i64'` (verified 2026-09-03). That fix taught the typechecker enough about associated functions on primitives to REJECT the unknown ones; this row is the other half, where the name is legitimately implemented so there is correctly nothing to diagnose, and yet no backend can execute it. Arguably the worse half for that reason: E0236 proves a table of valid associated functions on `i64` exists and that a user's `impl` adds to it. WORKAROUND: reach it through a generic (path (b)) or use a free function; that is why this is medium and not high. Surfaced authoring kata:312 (Burst Balloons) while probing a generic interval-DP table `Table[T]` whose `build()` needed a `T.zero()` seed -- the generic call inside the impl worked and the concrete `i64.zero()` written alongside it did not, which is what made the discriminator visible. | src/codegen/method_call.rs (compile_method_call dispatch) + the resolver's type-name-as-receiver path |
| B-2026-09-03-18 | 2026-09-03 | runtime | high | THE DEFAULT AUTO-PAR BUILD IS 123x SLOWER THAN THE SEQUENTIAL BUILD on a natural interval DP, and 45x slower than THE SAME BINARY run with `KARAC_PAR_WORKERS` set to the value auto-detect would have chosen anyway. `resolve_pool_workers()` (runtime/src/lib.rs) is called ONCE PER `karac_par_reduce` DISPATCH -- deliberately, and its doc comment justifies that as "cheap libc getenv". That is true only on the env-SET path, which early-returns. On the env-UNSET path -- the DEFAULT, what every user gets -- it falls through to `thread::available_parallelism()`, which on Linux probes the cgroup CPU quota through the FILESYSTEM. MEASURED (container x86-64, 4 cores, kata:312 bench workload: one 300-balloon row, 88 full interval-DP solves over a 302x302 table): sequential build (`KARAC_AUTO_PAR=0`) 0.553s; default auto-par build 67.8s / 68.1s on two runs (user 15.9s, SYS 51.8s -- system-dominated); the same auto-par binary under `KARAC_PAR_WORKERS=4` 1.51s / 1.46s (sys 0.00s). A worker sweep is flat and fast at every explicit value -- 1: 1.55s, 2: 3.85s, 3: 1.64s, 4: 1.50s, 8: 1.46s -- so the pathology is not a worker COUNT, it is the env var being unset. ROOT CAUSE CONFIRMED BY STRACE, not inferred. On a 2-pass variant of the same workload the default run makes 90,301 `sched_getaffinity` calls and 270,903 `openat`/`close`/`lseek` plus 722,415 `read` and 361,204 `statx`. The opened paths are exactly three, each hit 90,301 times: `/proc/self/cgroup`, `/sys/fs/cgroup/cpu/cpu.cfs_quota_us`, `/sys/fs/cgroup/cpu/cpu.cfs_period_us`. So `available_parallelism()` costs ~3 file opens + ~8 reads + ~4 statx PER CALL. The call count pins the granularity precisely: the DP has sum_{t=1..300} t = 45,150 (i, j) cells per pass, and 90,301 = 2 x 45,150 + 1 (the +1 being pool init) -- i.e. EXACTLY ONE cgroup probe per parallel region entry. NOT THE MECHANISMS ONE WOULD GUESS. Ruled out by measurement: (a) atomic promotion -- `KARAC_PAR_ATOMIC_PROMOTION=0` still takes 67.6s, so it is not the shared-write lowering; (b) worker contention / futex storms -- an explicit `KARAC_PAR_WORKERS=4` on the SAME binary is 1.51s with zero sys time, so the pool itself is fine; (c) fine-grained regions per se -- a minimal repro with a small distinct-index-write inner loop entered 300 times shows only ~50us per region, two orders of magnitude below this workload's ~2.5ms, and the difference is the probe count. CORRECTNESS IS UNAFFECTED, which is why no A/B check catches it: all four kata arms are byte-identical under `karac run`, `karac build` and the default auto-par build, and the bench workload prints the same `checksum 289414141` on all five languages and in every mode. This is purely a perf cliff, and it is invisible to the entire existing test posture. FIX DIRECTION (one-line-ish): cache the AUTO-DETECT result, not the env read. A `OnceLock<usize>` around the `available_parallelism().unwrap_or(4).max(2)` tier preserves everything the doc comment says it wants -- the env var is still read on each call, so a user can still override the count for a single command-line invocation without rebuilding -- while removing the filesystem probe from the hot path. Alternatively hoist the whole `resolve_pool_workers()` call out of `karac_par_reduce_pooled` and pass the pool's own worker count (the pool already resolved it once at `OnceLock::get_or_init`), which is strictly better but touches the dispatch-math comment's stated invariant about the two counts matching. IMPACT IS NOT KATA-SPECIFIC: any auto-par program whose parallel regions are numerous pays this, and the cost scales with the NUMBER OF DISPATCHES rather than with the work, so the finer the parallelism the worse the ratio. Also note the second-order consequence for benchmarking: this was found because a kara-katas bench harness bug left an auto-par binary in the path the sequential runtime lane measures, turning a 570ms lane into a 68s one -- fixed separately in kara-katas (scripts/new-bench.sh + 135 generated harnesses). | runtime/src/lib.rs resolve_pool_workers() + its per-call read in karac_par_reduce_pooled |
