...
| B-2026-09-15-6 | 2026-09-15 | codegen | medium | TWO SPELLINGS OF ONE READ WORKLOAD DIFFER BY ~47-51 POINTS UNDER SSO, ON EVERY HOST TESTED -- `substr` (via `String.substring`) against `lexlike` (via slice syntax) do identical work (slice a 3-byte token, compare to a keyword, discard, no consuming call) and the GAP between them is stable at 46-48 pts on one container and 49-51 pts on another, while NEITHER RAIL'S OWN DELTA IS PORTABLE -- `lexlike` is -13% on one host and +23% on the other, same compiler. The differential is the finding; the levels are not. Never profiled. Reachable only at KARAC_SSO=1, off by default. | — |
