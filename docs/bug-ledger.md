...
| B-2026-09-19-45 | 2026-09-19 | codegen | medium | A DECLARED `Array[E, N]` ENUM PAYLOAD WHOSE ELEMENT IS A USER ENUM STRANDS THE ELEMENT'S HEAP -- `enum Ha { P(Array[Mono, 1]), Q }` over `enum Mono { P(R2), Q }` loses 80 B + 11 B at `-O0` while the GENERIC twin `G[Array[Mono, 1]]`, the same element type and count, is clean, and so is the `Vec[Mono]` payload beside it | — |
...
| B-2026-08-28-76 | 2026-08-28 | autopar | high | AUTO-PAR UNDER-RETURNS ON THE M5: kata:288 now 3.45x and kata:282 3.09x for ~15.7 cores, against 3.06x/3.87x on 4 HOMOGENEOUS container cores. FILED AT 1.08x ON kata:288 -- most of that gap was f-string `snprintf` serialization (B-2026-09-05-23, fixed), NOT the static partition this row originally blamed; kata:282 does not format and is unchanged | B-2026-09-12-28 + B-2026-09-13-22 |
| B-2026-08-28-77 | 2026-08-28 | codegen | medium | kata:895 IS A COMPILER REGRESSION, NOT A HOST ONE -- the container re-run this row asked for twice gives 50.77ms today against the 26.50ms recorded 2026-08-18, while rust_ovf (34.71->34.24), c (6.58->6.55) and go (27.77->27.73) all reproduce within 1.5% on IDENTICAL toolchains, so kara alone doubled. Today's container ratios (vs rust_ovf 1.48, vs c 7.76) sit with the M5's (1.76, 7.89), not with its own baseline's (0.76, 4.03): the split is by COMPILER VERSION, not host. The 2026-08-18 baseline predates the SipHash migration (59c8d30cd, 2026-08-22) by four days. Of the 153.8M-instruction gap to equal-safety Rust, ALLOCATOR TRAFFIC is the largest identified component (49.6M vs 4.0M, 12.4x -- confirming this row's own churn hypothesis) ahead of the hash's per-call seed read (30.9M, 20% -- NOT an FFI cost: the boundary is ~1 instruction); an earlier 'the hash IS the gap' reading is WITHDRAWN in the detail. Cross-refs B-2026-09-07-42 / B-2026-09-07-53; the aarch64 hash-tag suspect was doubly dead | B-2026-09-07-53 |
...
| B-2026-09-12-29 | 2026-09-12 | codegen | medium | The stack-boxed enum payload covers Map.get only; Vec.pop / Vec.first / a -> Option[Wide] return still heap-box per iteration |
| B-2026-09-13-31 | 2026-09-13 | codegen | medium | Three map katas are 5-8% slower under the group-scan default, and no runtime signal is known that would route them back to the byte walk |
...
| B-2026-08-30-53 | codegen | medium | The SAME `match` arm as B-2026-08-29-58 with the arm NOT TAKEN loses the assigned-into local's OWN initializer `Drop` body on both compiled backends… | ed0a330b |
| B-2026-08-30-54 | interp+codegen | medium | The FIELD-target spelling `h.f = r` of B-2026-08-29-58 is wrong on BOTH backends in different directions: the interpreter runs the payload's `Drop` b… | 9a8b4616 |
...
| B-2026-08-31-28 | interp+codegen | medium | A BARE `match` ARM HANDING OUT A HEAP-CARRYING `Option` PAYLOAD RUNS NO `Drop` BODY ON EITHER COMPILED BACKEND -- `let _ = match o { Some(r) => r, No… | 8ccadea |
...
| B-2026-09-01-9 | codegen | low | A BRANCH MIXING AN F-STRING ARM WITH A STRING-LITERAL ARM LEAKS THE F-STRING -- `if c { f"i{n}" } else { "lit" }.contains("i")` strands the 2 B buffe… | 5671b4d |
...
| B-2026-09-01-36 | cli+codegen | medium | A REPL CELL BINDING WHOSE OWN TYPE IS `shared` CANNOT JOIN THE JIT SNAPSHOT TIER: admitted, `s.n` reads the POINTER'S OWN BITS instead of the field (… | 89c1435 |
| B-2026-09-01-37 | cli | medium | A STRUCT LITERAL USED AS A CALL ARGUMENT, ALONE IN A REPL CELL, SILENTLY PRODUCES NO OUTPUT on BOTH backends -- `println(f"{take(H { n: 1 })}")` prin… | 800c58b |
...
| B-2026-09-01-39 | interp+codegen | medium | A LIVE LOCAL HANDED OUT OF A DISCARDED BRANCH LOSES ITS PAYLOAD'S `Drop` BODY, and the `if` and `match` spellings disagree in OPPOSITE directions on… | 57633d2 |
...
| B-2026-09-01-43 | typecheck | medium | `partial_move_of_drop_struct` IS REGISTERED AT `Warn` WHERE design.md SAYS "rejected" -- eight `--features llvm` fixtures use the shape, and seven of… | 9bf4f53c |
...
| B-2026-09-02-22 | codegen | low | A DOUBLY-NESTED `Option[Option[String]]` ARGUMENT LEAKS ITS INNER PAYLOAD -- 45 B in 3 blocks over a three-iteration loop, unmoved by all THREE of B-… | 744b90a |
...
| B-2026-09-03-4 | interp+codegen | medium | A MATCH ARM (OR `let`) THAT RETURNS ITS ELEMENT WRAPPED IN AN ENUM CONSTRUCTOR RUNS THE `Drop` BODY TWICE AND THE CALLER'S BINDING NEVER RUNS ITS OWN… | 38e8666 |
...
| B-2026-09-03-7 | interp+codegen | medium | A BY-VALUE PARAM DESTRUCTURE INSIDE A METHOD PLACES THE LEAF'S `Drop` BODY AT THE CALLEE'S SCOPE EXIT ON THE COMPILED BACKENDS AND AT THE LEAF'S NLL… | edb7236 |
...
| B-2026-09-03-28 | codegen | medium | A TUPLE REBIND LEAKS ITS `Option` PAYLOAD ONLY WHEN OTHER TUPLE SHAPES ARE DEFINED IN THE SAME FILE -- `let t = (0, Option.Some(mkD(8))); let t2 = t;… | cd49671 |
| B-2026-09-03-30 | interp | medium | A FRESH VALUE ASSIGNED OVER A PARAM VIEW LOSES ITS SCOPE-EXIT `Drop` BODY UNDER `--interp` -- `a = h; a = R{5}` prints `dR5` on both compiled backend… | 4a1729f |
...
| B-2026-09-04-1 | codegen | medium | A `Result` DESTRUCTURE LEAF CONSUMED BY A MATCH LOSES THE PAYLOAD'S `Drop` BODY ON THE COMPILED BACKENDS -- `let HoRes { a, b } = h; match b { Result… | bfa5876 |
...
| B-2026-09-04-3 | codegen | medium | A `shared struct` HELD IN A TUPLE ELEMENT RUNS NO `Drop` BODY ON ANY OF THE THREE BACKENDS, and MEMORY IS BALANCED -- the box and its `String` are bo… | f82a99b |
| B-2026-09-04-4 | codegen | medium | A GENERIC STRUCT'S `impl[T] Drop for S[T]` STILL RUNS NO BODY ON THE COMPILED BACKENDS -- `--interp` prints `dB`, `karac build` prints nothing, and t… | a036937 |
...
| B-2026-09-04-6 | other | low | design.md CONTRADICTS ITSELF ON `#[derive(Copy)]` WITHOUT `Clone`, and the half that is wrong is the one a reader looking up `Copy` will find -- § De… | cbcd9f7 |
...
| B-2026-09-04-11 | codegen | low | THE TWO `uam_*` SPAN SETS STILL LEAK FROM THE USER PROGRAM INTO THE BAKED-STDLIB BODY PASS -- `Span` carries no file identity, so `uam_consume_sites`… | 1db0bb9 |
...
| B-2026-09-04-16 | codegen | medium | CODEGEN'S `Map[i64, Vec[i64]].keys()` ITERATES SORTED AND SEED-INDEPENDENT -- diverging from the interpreter's correct random order -- WHENEVER A STR… | c7c1a67 |
| B-2026-09-04-17 | codegen | medium | WHETHER A TUPLE, `Array` OR `Option` FIELD OF A `shared struct` IS COPIED OR MOVED ON READ IS UNMEASURED, so B-2026-09-04-15's rejection deliberately… | b72bef2c |
| B-2026-09-04-18 | codegen | high | A `Result[<inline struct with a Drop body>, _]` LOCAL ABORTS ON REBIND AND PHANTOM-DROPS ON AN ARM-TAIL ESCAPE -- `let r: Result[R, String] = Result.… | bfa5876 |
...
| B-2026-09-04-23 | codegen | low | A DISCARDED FIELD AND AN UNUSED LEAF DROPPED AT THE SAME DESTRUCTURE RUN THEIR BODIES IN OPPOSITE ORDER ON THE TWO BACKENDS -- `let HoRes { a: _, b:… | a66361a |
...
| B-2026-09-04-32 | codegen+other | low | EVERY COMPILED BACKEND RELEASES AN AGGREGATE-HELD `shared` FIELD AT LEXICAL SCOPE EXIT while design.md pins RC decrements at the binding's LIVE-RANGE… | 61d7477aa |
...
| B-2026-09-04-36 | interp+codegen | low | A RECEIVER TEMP NESTED IN A LARGER EXPRESSION DRAINS AT THE STATEMENT'S `;` ON THE COMPILED BACKENDS AND AT THE CALL RETURN IN THE INTERPRETER -- `pr… | dd2a7965a |
...
| B-2026-09-05-7 | codegen | medium | A GENERIC-INSTANTIATION LEAF BOUND OUT OF A BY-VALUE PARAM DESTRUCTURE RUNS NO `Drop` BODY -- `fn gDes[T](h: Gn2[T]) -> i64 { let Gn2 { inner, z } =… | 51368a1 |
...
| B-2026-09-05-15 | codegen | medium | A NESTED GENERIC-INSTANTIATION FIELD WITH ITS OWN `impl[T] Drop` LOSES THAT FIELD'S BODY UNDER AN OWN-`Drop` PARENT -- `oOwn(Gouter[R] { inner: Go[R]… | 4ff27de |
...
| B-2026-09-05-17 | interp+codegen | medium | A FORWARDING CALLEE DEFEATS THE ESCAPING-FIELD MASK, so a place struct argument whose field is handed back THROUGH ANOTHER CALL still runs that field… | 91b8155 |
...
| B-2026-09-06-4 | codegen | high | THE SELF-HOSTED RESOLVER ORACLE DOUBLE-FREES ON LINUX AND THE EMITTER ORACLE SEGFAULTS, AT THE IMPORT COMMIT AND ON CURRENT `main` ALIKE -- `tests/se… | 98a8251 |
...
| B-2026-09-06-8 | codegen | medium | PROJECTING THE `Drop` ELEMENT OUT OF A TUPLE PARAM (`let x: R = t.0`) LEAKS `x`'S INTERIOR AT -O0 ON EVERY COMPILED BACKEND -- 10 B in 2 blocks per c… | 92350b6 |
| B-2026-09-06-9 | interp+codegen | medium | A BY-VALUE `Drop` PARAM HANDED TO A CALLEE THAT RETURNS IT, WITH THE RESULT BOUND TO A LOCAL, RUNS THE BODY TWICE ON ALL FOUR SURFACES -- `fn s_keep(… | 9579de0 |
...
| B-2026-09-06-11 | interp+codegen | medium | A TUPLE ARGUMENT WHOSE CALLEE HANDS BACK A PART BELOW ITS TOP-LEVEL ELEMENTS RUNS THAT PART'S `Drop` BODY TWICE ON ALL FOUR SURFACES, NAMED LOCAL AND… | df9e581 |
| B-2026-09-06-12 | interp+codegen | medium | A BY-VALUE `Drop` PARAM REBOUND THROUGH AN ALWAYS-RETURNING CALLEE AND THEN RETURNED RUNS THE BODY TWICE ON ALL FOUR SURFACES -- `fn s_keep_ret(r: R)… | 1388f70 |
| B-2026-09-06-13 | interp+codegen | medium | A BY-VALUE `Drop` PARAM HANDED TO A CALLEE THAT RETURNS IT ON SOME PATHS, WITH THE RESULT BOUND TO A LOCAL, RUNS THE BODY TWICE ON BOTH PATHS -- `fn… | f7f69f4 |
| B-2026-09-06-14 | typecheck | low | W0299 `borrow_projection_copy` IS SILENT FOR A `match` SCRUTINEE THAT PROJECTS OFF A BORROW -- `match h.e { E.A(r) => { let m = r; . | eb183c2 |
| B-2026-09-06-15 | interp+codegen | medium | A BARE `match self { H1 { e } => . | 056410e |
| B-2026-09-06-16 | interp+codegen | medium | `let e = self.e` INSIDE AN OWNED RECEIVER RUNS BOTH THE FIELD'S AND ITS PAYLOAD'S `Drop` BODIES TWICE FOR A NAMED-LOCAL RECEIVER -- `dR51 dE dE dR51`… | 9efbc0d |
...
| B-2026-09-06-27 | interp+codegen | medium | A READ-ONLY ARM ON AN OWNED ENUM RECEIVER LOSES THE PAYLOAD'S `Drop` BODY UNDER `--interp` -- `impl E { fn m_read(self) -> i64 { match self { E.A(r)… | c87f945 |
...
| B-2026-09-06-35 | interp+codegen | low | A PARTIAL STRUCT `match` PATTERN DROPS THE `..` REST FIELD BEFORE THE BOUND FIELD ON THE INTERPRETER AND AFTER IT ON EVERY COMPILED BACKEND -- `let s… | edbfa5209 |
...
| B-2026-09-06-59 | codegen | low | READING A DECLINED-COPY PARAM AFTER A WHOLE REBIND LEAVES ITS `String` UNOWNED -- `fn f(r: R) -> String { let m = r; return f"{r.name}"; }` over a st… | d2edcbe85 |
| B-2026-09-06-60 | codegen | high | A BY-VALUE PARAM WHOSE STRUCT HAS A DIRECT `Map` FIELD DOUBLE-FREES WITH NO REBIND AT ALL -- `fn norebind(q: Q) -> i64 { return q.id; }` over `struct… | fe99193 |
...
| B-2026-09-06-63 | interp+codegen | low | A CALLEE THAT WRAPS A `Drop`-BEARING ARGUMENT IN ANOTHER `Drop`-BEARING TYPE LOSES THE WRAPPER'S OWN BODY -- `fn wrap_bodied(r: R) -> H { return H {… | 2b52d0a |
...
| B-2026-09-06-65 | interp+codegen | low | A PLAIN OWNED-`self` METHOD ON A FRESH TEMP RUNS THE RECEIVER'S `Drop` BODY BEFORE THE CALL'S RESULT IS PRINTED ON THE INTERPRETER AND AFTER IT ON EV… | dd2a7965a |
...
| B-2026-09-07-6 | interp+codegen | medium | A WHOLE-TUPLE ARGUMENT TO A PASSTHROUGH METHOD OR ASSOC FN LOSES ITS ELEMENT'S `Drop` BODY ON EVERY COMPILED BACKEND -- `impl Hold { fn thrut(ref sel… | 093de9e |
...
| B-2026-09-07-8 | codegen | high | A MIXED-PATH CALLEE REACHED WITH THE ENCLOSING FRAME'S OWN DECLINED-COPY PARAM DOUBLE-FREES, and the REBOUND spelling additionally runs one EXTRA `Dr… | d3a2469 |
...
| B-2026-09-07-21 | codegen | low | TWO DISCARDED-LITERAL FIELD SHAPES STILL HAVE NO OWNER AFTER B-2026-09-01-5 -- a projecting arm followed by another statement strands 38 B in the SEQ… | ee14f8aa6 |
...
| B-2026-09-07-30 | codegen | medium | A PROJECTION OFF AN RC-PROMOTED LOCAL DOUBLE-FREES WHEN ITS DESTINATION IS A `Vec.push` ARGUMENT OR AN EXISTING BINDING -- `v.push(t.a)` in a loop gi… | 13fe42c |
...
| B-2026-09-07-41 | codegen | low | A WHOLE-VALUE REBIND OF A BY-VALUE ENUM PARAM STRANDS ITS PAYLOAD AT -O0 -- `fn rebind(w: W) -> i64 { let v = w; return match v { W.T(x) => x.a.unwra… | 2a982fd85 |
| B-2026-09-07-42 | codegen | high | PERF-REGRESSION, CORPUS-WIDE AND UNNOTICED SINCE ~2026-08: 12 of the 23 map-bearing benched katas whose recorded-run binary survives execute 1.30x-4.… | 9ce695e9e |
| B-2026-09-07-43 | codegen | medium | THE IF/ELSE SPELLING OF A DISCARDED AGGREGATE LITERAL NEVER REACHES THE OWNER REGISTRAR, so an RC-boxed projection's COPY is stranded -- 190 B in 5 b… | d0e5077 |
| B-2026-09-07-44 | codegen | low | A FRESH-TEMP ENUM SCRUTINEE WHOSE CONSUMING ARM BINDS A COPY-DECLINED STRUCT PAYLOAD LEAKS THE PAYLOAD'S INTERIOR -- `match W.T(mkx(1)) { W.T(x) => . | 318f8a6c1 |
...
| B-2026-09-07-53 | codegen | high | AT EQUAL HASHING kara's Map IS NOW SLOWER THAN RUST'S, INVERTING A PUBLISHED HEADLINE: on 3 of 4 re-measured map katas kara went from 3-4x AHEAD of b… | fd6750a84 |
...
| B-2026-09-07-58 | codegen | medium | AN RC-PROMOTED BINDING PUBLISHED THROUGH A VALUE-PRODUCING `par` BLOCK'S JOIN STILL LOSES ITS BOX -- `let (t, k) = par { let t = mkp(9); .. | a701a3266 |
| B-2026-09-07-62 | codegen | medium | A BOXED SHARED-ENUM PAYLOAD'S NESTED STRUCT FIELD NEVER HAS ITS BUFFERS FREED UNLESS THAT FIELD IS MOVED OUT -- 32 B leaked with no move-out and with… | 8bc213c |
...
| B-2026-09-08-3 | codegen+interp | medium | A METHOD-SIDE `self.f = <new>` CANNOT SEE THE CALLER'S MOVE-OUT OF `f`, so the displacement fires over a husk the caller already handed away -- `let… | f39940a |
...
| B-2026-09-08-5 | codegen | low | THE B-2026-09-07-63 RE-ARM IS INNERMOST-FRAME ONLY, so a reassign one frame deeper still loses the new value's `Drop` body -- `if f { g.one = mks(7);… | 9271808 |
...
| B-2026-09-08-9 | codegen | medium | A TWO-HOP FIELD MOVE-OUT IN A LOOP OFF AN RC-PROMOTED ROOT DOUBLE-FREES ON THE JIT AND READS A JUNK LEAF AT -O0 -- `while i < 1 { let x = o.h.r; }` a… | e6a109a |
...
| B-2026-09-08-11 | codegen | low | `display_mangle_te` RENDERS EVERY UNNAMEABLE TYPE AS `unknown`, SO THEY SHARE ONE MANGLED SYMBOL AND ONE DROP/CLONE CACHE ENTRY -- `Vec[weak A]` and… | d9e609d64 |
| B-2026-09-08-12 | other | low | THE INSTRUMENTED ASAN LEG REPORTS CLEAN ON A LEAK THE DEFAULT LEG CATCHES, CONTRADICTING ITS OWN DOCUMENTED SUPERSET INVARIANT -- measured on two B-2… | 3ecdfc162 |
| B-2026-09-08-13 | codegen | medium | A LET-BOUND LOCAL OF A SELF-REFERENTIAL STRUCT PASSED BY VALUE LOSES ITS `Drop` BODY ON EVERY COMPILED BACKEND -- `let c = mkn(10); read(c)` prints t… | 3f2342c |
...
| B-2026-09-09-7 | other | medium | A FULL DISK FAILS THE GATE SET AS AN LLVM CRASH, A LINKER BUS ERROR, OR A LOST OUTPUT STREAM -- never as a named test -- and it hit ELEVEN times acro… | ba74971 |
...
| B-2026-09-09-12 | runtime | medium | kara's MAP PROBE WALKS ONE CONTROL BYTE PER STEP WITH A DATA-DEPENDENT BRANCH, and an 8-byte SWAR group scan is 2.03x FASTER IN CYCLES WHILE EXECUTIN… | 6cc029853 |
...
| B-2026-09-09-24 | codegen | low | AN `Array` PAYLOAD BINDING THAT IS INDEXED STRANDS ITS ELEMENTS AT `-O0` -- 18 B in 2 blocks for `Some(t) => t[0]` over `Option[Array[String, 2]]` an… | 4dc4bdf4e |
...
| B-2026-09-10-7 | codegen+interp | low | AN ARM-BOUND `Array` PAYLOAD NEVER RUNS ITS ELEMENTS' `Drop` BODIES, and the rebind spelling runs them on the COMPILED backends only -- `Some(t) => {… | 6aba597c9 |
...
| B-2026-09-10-14 | codegen+interp | low | A WHOLE-PAYLOAD ARM BINDING OVER A LOCAL `Option[(R, R)]` RUNS NO ELEMENT `Drop` BODY ON EITHER BACKEND -- `match o { Some(t) => { println("hit") } . | 813c441 |
...
| B-2026-09-12-5 | codegen | medium | A BOXED TUPLE / `Array` / GENERIC-STRUCT PAYLOAD MATCHED OUT OF A GENERIC ENUM WAS OWNED BY NOBODY -- `clear_boxed_enum_inner_drop` retracted the box… | 68177de |
...
| B-2026-09-12-17 | codegen+interp | medium | A GENERIC USER ENUM'S PAYLOAD `Drop` BODY RUNS ON NO COMPILED BACKEND WHEN THE ARGUMENT IS A FRESH TEMP -- `takeit(Full(R { id: 5 }))` over `enum Ho[… | 6a27618 |
...
| B-2026-09-12-26 | codegen | medium | A CALLEE WHOSE LAST EXIT IS A TAIL `return` (NO TRAILING SEMICOLON) IS INVISIBLE TO `fn_conditionally_returns_param_bare`, so `impl Sk { fn pick(r: R… | 6de686c |
...
| B-2026-09-12-28 | codegen | high | Map.get's returned enum is heap-boxed and freed per iteration even when the match only reads it | ef9a76f90 |
...
| B-2026-09-13-2 | codegen | medium | THE `Option[Array[T, N]]` A `Map` HANDS BACK IS OWNED BY NOBODY -- `insert`'s displaced old value leaks 336 B in 14 blocks and `remove`'s return 192… | 7705fe2 |
...
| B-2026-09-13-4 | other | low | THE DIFFERENTIAL CANNOT COMPARE A MATCHED PARAMETER'S PAYLOAD AT ALL, so B-2026-09-12-27's now-correct schedule for that population is unwatched -- r… | da441ce |
| B-2026-09-13-5 | interp | medium | THE INTERPRETER RUNS A `Drop` BODY TWICE WHEN AN ARM MOVES A SUB-VALUE OUT OF A BY-VALUE `Option` PAYLOAD, and the COMPILED backends are the correct… | 1f02b92 |
| B-2026-09-13-6 | codegen | medium | A USER STRUCT USED AS A `Map` KEY LEAKS THE ORPHANED DUPLICATE -- 252 B in 14 blocks for a repeated `struct K { a: String, b: String }` key, the same… | b76fdfa |
| B-2026-09-13-8 | other | medium | TWO SESSIONS FIXED ONE CLAIMED LEDGER ROW IN PARALLEL AND THE SECOND LANDED A REGRESSION BEHIND A VACUOUS GREEN -- B-2026-09-12-25 was tagged `kara-b… | 4a32ad2 |
| B-2026-09-13-9 | codegen | high | A NESTED DESTRUCTURE OF A BOXED ENUM PAYLOAD STILL DOUBLE-FREES WHEN THE LEAF IS WIDER THAN THE ENVELOPE'S PAYLOAD AREA AND THE ARM REBINDS OR MOVES… | 7c79677 |
...
| B-2026-09-13-11 | codegen+interp | low | A DESTRUCTURED ENUM PAYLOAD'S OWN `Drop` BODY RUNS ON NO SURFACE ONCE THE ARM MOVES ITS FIELD OUT -- `match x { Option.Some(K.A(r)) => { acc.push(r)… | 66149c2 |
| B-2026-09-13-12 | codegen+interp | medium | THE let-else SPELLING OF A NESTED ENUM-PAYLOAD DESTRUCTURE LOSES THE PAYLOAD ENUM'S `Drop` BODY ON EVERY COMPILED BACKEND -- `fn give(x: Option[K]) -… | 66149c2 |
| B-2026-09-13-13 | codegen | medium | A CONDITIONALLY-RETURNED PARAM WHOSE OTHER EXIT LEAF MENTIONS IT STILL RUNS ITS `Drop` BODY TWICE AT THE ASSOCIATED AND METHOD CALL POSITIONS -- `if… | 35eef75 |
| B-2026-09-13-14 | typecheck | medium | PROMOTE `partial_move_of_drop_enum` FROM `Warn` TO `Deny`, which is what actually removes B-2026-09-13-12's run-vs-build divergence -- blocked on tri… | 66149c2 |
...
| B-2026-09-13-17 | codegen | medium | `Vec.pop()` HANDS BACK AN `Array[T, N]` ELEMENT NOBODY OWNS -- 176 B in 8 blocks over four pops of a `Vec[Array[String, 2]]`, the same numbers and th… | b7e88b3 |
...
| B-2026-09-13-23 | codegen | low | AN `Array[String, N]` INSIDE A TUPLE LEAKS ITS ELEMENT BUFFERS -- 20 B in 2 blocks at -O0 for `let t: (Array[String, 2], i64) = ([f"a{n}", f"b{n}"],… | d1e6479 |
...
| B-2026-09-14-5 | codegen | medium | THE COMPILED BACKENDS LOSE AN OWED `Option` PAYLOAD BODY FOR AN OWNED PARAM -- `fn eat(o: Option[(R, i64)]) -> i64 { match o { Some(t) => return t.1… | 7ae694f |
...
| B-2026-09-15-21 | codegen | medium | AN ARM-BOUND PAYLOAD ASSIGNED OVER A `mut` LOCAL STRANDS THE DISPLACED VALUE'S HEAP FIELD, BUT ONLY WHEN THE ENUM ARRIVED AS A BY-VALUE PARAM -- `out… | c4763b7 |
...
| B-2026-09-15-24 | interp+codegen | medium | A USER-DEFINED METHOD THAT DISCARDS A BORROW-PROJECTION ARGUMENT RUNS THE FIELD'S `Drop` BODY TWICE UNDER `--interp` AND ONCE ON EVERY COMPILED SURFA… | 3cad6b5 |
...
| B-2026-09-16-2 | codegen+interp | medium | THE DISPLACED ELEMENT'S USER `Drop` BODY NEVER RUNS FOR A TUPLE OR NESTED-ARRAY ELEMENT SHAPE, ON EITHER BACKEND -- `a[0] = <new>` over `Array[(D, i6… | 021b83f |
...
| B-2026-09-16-35 | other | medium | the ASAN suite has never run at `KARAC_SSO=1`, so the entire inline-String surface — including a codegen-owned heap allocation — has no sanitizer cov… | b8eeb34 |
...
| B-2026-09-17-38 | codegen | medium | A `Drop`-BEARING SIBLING PART IS LOST ON EVERY COMPILED SURFACE WHEN ITS PEER IS CONSUMED BY AN IN-FRAME LOCAL -- `fn eat(o: Option[(R, R)]) { match… | 0518234 |
