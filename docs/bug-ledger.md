...
| B-2026-09-15-6 | 2026-09-15 | codegen | medium | TWO SPELLINGS OF ONE READ WORKLOAD DIFFER BY ~46-52 POINTS UNDER SSO ON x86-64 AND BY 3 POINTS ON arm64 -- `substr` (via `String.substring`) against `lexlike` (via slice syntax) do identical work (slice a 3-byte token, compare to a keyword, discard, no consuming call) and the GAP between them is stable at 46-48 pts on one x86-64 container (2 samples) and 48.8-51.9 pts on another (5 samples), but is 3 pts on an M5 Pro where BOTH spellings win ~50%, while NEITHER RAIL'S OWN DELTA IS PORTABLE -- `lexlike` is -13% on one host and +23% on the other, same compiler. The differential is the finding; the levels are not. Never profiled. Reachable only at KARAC_SSO=1, off by default. | — |
