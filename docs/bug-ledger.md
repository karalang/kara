...
| B-2026-08-15-19 | autopar | medium | `#[par_order_free]` IS SILENTLY IGNORED when the analyzer does not classify the loop as a collect: no fan-out, no diagnostic, and `karac query concur… | FIXED by b5af2c1. (Filed as a now-dangling short sha in the close commit's subject; that was the
