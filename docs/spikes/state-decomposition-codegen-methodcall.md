...
**Status:** ✅ **COMPLETE — Phase 1 (`infer_method_call` 5,873 → 694 lines, −88%, 13 slices); Phase 2 (`Codegen` 439 → 81 fields, −82%, 23 clusters across 14 named slices + 9 cluster-15 sub-slices). The 58-field residual is declared and documented in the status table: the LlvmCore substrate, `variables` (the frame core), and the genuine singles. Every subsystem now lives in a named module.** This doc is the live coordination point for the refactor: the cluster map below is measured (not guessed), and the status table at the bottom is updated as slices land. **Other agents: read § Coordination protocol before editing `src/codegen.rs` or `src/typechecker/expr_method_call.rs`.**
...
| 2 | cluster 15 `VarTables` — sub-slice F **`RcElision`** (`src/codegen/rc_elision.rs`) | **landed** — 7 fields, the fn-keyed products of `src/ownership/elision.rs` (phase A `elided_bindings`, B1 `elided_cluster_roots` + `adopted_cluster_roots` + `conditional_adopted_roots` + `weak_targeted_types`, B2 `elided_b2_bindings`) plus `borrowed_param_skips`. No `'ctx`. 18 access sites; `Codegen` 134 → 128 fields | 2026-08-16 |
| 2 | cluster 15 `VarTables` — sub-slice G **`SpanTables`** (`src/codegen/span_tables.rs`) | **landed** — 33 fields: the `(offset, len)`-keyed side tables the lowering pass copies out of the typechecker's `Program` (typed-expr classifications, method dispatch + callee types, `?`-conversion/payload types, temp-recv + iterator-terminal elem types, call-site type substitutions, unsigned classifications) plus the ownership pass's span verdicts (`uam_*`, `vec_index_*`). Codegen only reads these, except the tracing-stdlib swap window. No `'ctx`. 150 access sites; `Codegen` 128 → 96 fields | 2026-08-16 |
| 2 | cluster 15 `VarTables` — sub-slice H **`BorrowVars`** (`src/codegen/borrow_vars.rs`) | **landed** — 11 fields: `ref_params` / `signature_ref_params`, the ownership classifications (`owned_vecstr_params`, `owned_struct_params`), the borrow-view sets (`for_loop_borrow_vars`, `borrow_accessor_let_payload`, `for_loop_owned_agg_vars`, `borrowed_agg_payload_struct_vars`, `entry_slot_ref_vars`), and the `Option[shared]` heap pair (`var_option_shared_heap`, `ref_option_shared_heap`). 203 access sites; `Codegen` 96 → 86 fields | 2026-08-16 |
| 2 | cluster 15 `VarTables` — sub-slice I **`ModBindings`** (`src/codegen/mod_bindings.rs`) | **landed** — 6 fields: `consts`, `module_bindings` + `module_binding_types`, and the three module-init queues (`map_set` / `once` / `computed`). Named `mod_bindings` because `module_bindings.rs` is an existing behaviour module — the provider-collision class, caught by the pre-flight check this time. 34 access sites; `Codegen` 86 → 81 fields | 2026-08-16 |
| 2 | cluster 15 — **residual declared, Phase 2 COMPLETE** | The remaining 58 non-handle fields are the sanctioned residual: the **LlvmCore substrate** (~16: `context`/`builder`/`module`/`current_fn`/`target_data`, debug info, used-symbol sets, stdout/stderr + source globals, `static_init_fn`, linkage overrides), **`variables`** (the frame's core slot map — with every side table moved, it now IS the frame, and belongs next to `current_fn`), and ~40 genuine singles (staging cells, flags, small span sets) that form no coherent subsystem — forcing them into a "misc" struct would be a grab-bag with no architectural meaning, exactly what this spike argues against. Every subsystem now has a named home. RECORD CORRECTION: the SpanTables commit message (eb9e5d88) lists `iter_let_bindings` / `user_ref_method_names` / `user_ref_method_inner` among the moved fields — they were NOT in that slice's field list and remain on `Codegen`; the code and its 33-field/150-site verification are correct, only the message overstates. | 2026-08-16 |
