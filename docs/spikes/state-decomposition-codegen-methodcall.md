...
| 2 | cluster 15 `VarTables` — remainder | in progress — next: module-level bindings (6 fields, 28 sites; named `mod_bindings` because `module_bindings.rs` is an existing behaviour module), then `variables` + residual singles | — |
