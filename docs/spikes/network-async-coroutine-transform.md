...
| 5c (mechanism done; trigger remaining) | **Cooperative cancellation (activates slice 4's destroy edge).** *Done (`4186c3ac`, safe + inert + zero-regression):* the resume shim `__kara_coro_resume(handle, cancel)` checks `cancel` before resuming → `coro.destroy` (runs the per-park destroy edge) instead of resuming, reports Ready; the per-park destroy edge `park_slot_signal`s the completion slot so a cancelled coroutine's waiter wakes (else hang). Both inert until a cancel flag is set (dispatcher still passes a never-cancelled global); verified by `coro_e2e.rs::coroutine_cancel_mechanism_emitted`. *Remaining (the concurrency-sensitive TRIGGER half — do with review):* (1) **per-task cancel routing** — add a `cancel` field to `KaracParkSlot` + the coroutine parked record (`{poll_fn,state,cancel,token}`, token→field 3) + runtime `KaracParkedTask`, loaded from the slot via a `karac_runtime_park_slot_cancel_ptr` getter; `spawn_coro` binds `slot.cancel = &handle.cancel`; the dispatcher passes `task.cancel` instead of `&disp.cancel`. (2) **`TaskGroup.cancel()`** Kāra surface → `karac_runtime_taskgroup_cancel` sets each child's `cancel`. (3) **dispatcher cancel-sweep** for parked-but-never-ready (idle) coroutines (the hardest part — races the normal resume; dedup via `take_registration`). (4) the slice-4-deferred **live mid-flight-cancel ASAN+leak test**. (5) decide **defer-on-cancel** (the destroy edge currently skips `UserDefer`). Full breakdown: task #17. See § 6⅞ | ✅ mechanism / 2 trigger |
