...
**Result:** codegen E2E via JIT on Linux **1416 → 0 failed (2084/2084)**; AOT baseline unchanged (2084/2084); `lljit_prototype` 15/15, `lljit_e2e` 16/16. **Slice 2 (the proof gate) is met on Linux.** This is the first time the JIT lane has ever been exercised — and passed — off macOS.
...
5. **Flip `repl` + `test` to JIT-by-default.** Remove the `cfg(lljit_prototype)` gates in `cmd_repl` / `cmd_test`; keep a `--interp` escape hatch for compiler-internal/dev use. **Linux de-risking done 2026-07-07 (gated on slice 1 for the actual gate-removal):** the `karac test` JIT batch-runner path is green on Linux (`tests/cli.rs` 491/491 under `--features lljit_prototype`, JIT-default) and the repl JIT path is 22/23 — the one failure is `B-2026-07-07-6` (cross-type REPL rebind crashes the runner on Linux; narrow, tracked).
...
7. **Gates.** Full suite green via JIT on macOS arm64 **and** Linux; Linux/LSan leak gate; DWARF crash-diagnostics verified; publish the cold-start bench. **Linux status 2026-07-07:** codegen E2E 2084/2084, `karac test` batch 491/491, `lljit_prototype` 15/15, `lljit_e2e` 16/16, `karac_jit_runner_repl` 3/3, `repl_jit` 22/23 (1 open). fmt + clippy (`--features lljit_prototype --all-targets`) clean. **Not yet done:** macOS re-verify (the fixes are ELF-no-ops there but confirm), Linux/LSan run of the JIT lane, DWARF-on-JIT verification, cold-start bench.
...
- **Slice 5 (flip repl/test to JIT-default).** Gated on slice 1 (the gates to remove are `cfg(lljit_prototype)`). Linux-de-risked this session (491/491 `karac test` batch); clear `B-2026-07-07-6` (repl cross-type-rebind crash) first for a clean repl flip.
