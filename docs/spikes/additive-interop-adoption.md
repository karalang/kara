...
- **Remaining (v1 hardening / follow-ons, roadmap Tier 5 — no core capability missing):** Windows library artifacts + 5-target CI matrix, project-mode `[lib]` table, Rust-host `std`-collision smoothing, book chapter + A/B-verified snippet, and auto-boxing of `enum` / user-struct / deeper-nested returns (rejected today with a guided `#[repr(C)]` fix; a struct should just be `#[repr(C)]`, a boxed enum isn't C-usable without accessors).
