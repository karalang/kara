...
- **Remaining (v1 hardening / follow-ons, roadmap Tier 5 — no core capability missing):** 5-target CI matrix, book chapter + A/B-verified snippet, and auto-boxing of `enum` / user-struct / deeper-nested returns (rejected today with a guided `#[repr(C)]` fix; a struct should just be `#[repr(C)]`, a boxed enum isn't C-usable without accessors). *(Project-mode `[lib]` table + Windows artifacts + Rust-host collision note shipped 2026-07-08.)*
