...
**Status:** OPEN — Slices 0–3, 3½, and 5 landed (2026-07-08); Slice 4's `forget` primitive landed too. Shipped: framing + export-ABI spec + native library build mode (`.a`/`.so`/`.dylib`) + C-header emitter + producer-side effect policing + a C-and-Rust proof-point demo + the `forget[T]` move-out primitive (verified sound). **What remains is only Slice 4's full allocate→use→free *round-trip***, blocked on two foundations found while building it: raw-pointer instance methods (`.offset`/`.read`/`.write`) are spec'd-but-unimplemented (`B-2026-07-08-4`), and the auto-boxing/auto-destructor sugar is gated on the CI-scale ASAN/LSan differential corpus. The *consume* half already shipped; the *producer* half is now real down to the move primitive.
...
**Slice 4 — ownership handoff across the boundary (the soundness fork). ✅ ACCEPTANCE MET (2026-07-08) — `forget` + Path-A round-trip shipped; Path-B sugar is v1.x.**
*Update: the spike's Slice-4 acceptance (`forget` + a stated handoff rule + a working round-trip) is now MET. Path A (raw-pointer instance methods `.offset`/`.read`/`.write`, `B-2026-07-08-4` closed) makes the sound allocate→use→free round-trip expressible — verified end-to-end under ASAN/LeakSanitizer (no leak, no use-after-free). Path B (compiler auto-boxing + auto-destructors) is the additive v1.x ergonomic layer, gated on the CI-scale ASAN/LSan corpus per the owner decision.*
