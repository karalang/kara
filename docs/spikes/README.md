...
| spike | status |
|---|---|
| [reduce-elementwise-trait-unification.md](reduce-elementwise-trait-unification.md) | 🟡 **S0–S1 COMPLETE 2026-06-30 (`bcaff37d`, `73af27b0`, `7adcc380`, `f5ad6a4c`); S2–S6 open.** Unifies the three copy-pasted reduce/element-wise/ordering impls (Tensor, Column, `Stats.*`) behind one internal `ContainerAccess`/`ElemKind`/`ReduceOp`-parameterized kernel (S0–S5), then layers **user-extensible** `Reduce`/`ElementwiseMap`/`ElementwiseOrd` surface traits (S6, gated on a default-method-body + generic-trait-method spike). S0 = shared `ReduceOp` vocab + interpreter twin (`src/reduce_kernel.rs`); S1 = codegen `ContainerAccess` (dense + Arrow `bitmap`) + `emit_reduce_fold`/`emit_reduce_minmax` (+ `_gated`) in `src/codegen/kernel.rs` — all three surfaces' `sum`/`prod`/`mean`/`min`/`max` migrated, `emit_scalar_reduce_loop` deleted, Column shed ~150 lines. Byte-identical (oracle 1937/0, par 127/0). Closes the two open `std.stats` long-tail items (non-f64 elems + trait unification) and lands Column `median`/`quantile` codegen as S4 fallout. Next: S2 (f64-accumulator family: Column `mean`/`var`/`std` + Stats variance). S0–S5 is the committed spine; S6 may split off. |
