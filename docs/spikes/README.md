...
| spike | status |
|---|---|
| [gpu-wgsl-slice0.md](gpu-wgsl-slice0.md) | ⬜ **SKETCH 2026-06-29 — scoping only, no code.** Smallest end-to-end GPU spine (one `#[gpu]` element-wise kernel → generated WGSL → wgpu dispatch → readback) so the "weeks not days" GPU-codegen estimate has a concrete first increment (0a runtime spine → 0b WGSL codegen → 0c wire `gpu.dispatch`). Documents the standing tension: roadmap puts GPU codegen in the *build-once-in-self-hosted* bucket; this scopes the Rust-now alternative + trade-off. Front-end (FE-1–4 + SL-1) is done & enforced. **Gated on an explicit go** (0a adds `wgpu` to the runtime + reverses the roadmap default). Maps to CG-1/2/3 + SL-2. |
