...
| [per-layout-monomorphization.md](per-layout-monomorphization.md) | 🟦 **IN PROGRESS 2026-06-20 — slices 1–5 landed (axis scaffolding + forward arg-layout mono + SoA returns + multi-buffer `ref`/`mut ref` borrow forms + origin-only `soa_layouts` cutover; by-value AND by-ref SoA `Vec[E]` cross call boundaries regardless of param name, a builds-and-returns helper is monomorphized to return the receiving binding's layout, multiple SoA buffers of one type flow through shared by-ref helpers as distinct monomorphs, and a binding's physical layout is now a per-binding `LayoutId` value carrier — not a name-keyed `soa_layouts` lookup — retiring the redundant by-value param ABI + its base-symbol footgun, with the cross-group disjointness audit confirming the layout-agnostic borrow checker needs no new fact).** Make SoA `layout` a monomorphization axis so it crosses call boundaries (returns + differing-name/multi-buffer kernels). Layered on the generic-mono engine: keep `layout <name>` as the layout *origin*, add layout-flow inference (forward arg layouts incl. borrow forms, backward return layout from the receiving binding) + per-layout monomorph keying/mangling. Supersedes the name-keyed cross-fn slices. Tracks B-2026-06-19-14, design.md Feature 1 / P1.5 (Phase 11). 6-slice plan, each gated on the codegen suite + LSan; slice 6 (Slipstream proof) is blocked on a pre-existing field-level SoA index-store fix. |
