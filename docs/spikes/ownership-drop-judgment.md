...
- **Borrow-escape / closures. — spawn half MECHANIZED 2026-07-08.** The oracle now models the *drop-schedule* consequence of a `spawn` closure capture: a heap binding captured by a closure is demoted to `Borrowed` (it escapes as an auto-promoted shared/RC reference, so the enclosing scope no longer owns the free — the RC/join does), and later reads / additional captures of it are valid (no false use-after-move on the multi-`spawn` shape). This makes the oracle agree with codegen — which RC-promotes and frees via the join, not the binding's scope cleanup — so the differential now **checks** spawn programs instead of skipping them (corpus coverage 32% → ~99.7%, 0 divergences). Verified by `ownership_oracle::tests::spawn_captured_vec_is_not_scheduled_and_no_uam` and `tests/drop_differential.rs::spawn_capture_is_checked_clean`. **Still open:** `par {}`-block captures (next bullet) and the general borrow-*escape decision procedure* for stored/heap-env closures (entangled with the live ownership-checker false positive that gates heap-env closures out of the fuzzer) — the oracle's current rule is the conservative "any closure capture is a shared borrow," correct for the fuzzer's `spawn`/`par` escapes but not a full escape analysis.
