...
- **Borrow-escape / closures.** §3.3 states "a borrow must not outlive its source," and §5.4 attributes the closure soundness bugs to it, but the judgment does not yet give the *decision procedure* for when a closure capture is a borrow (must not escape) vs a by-move capture (Escape, §4). This is entangled with the live ownership-checker false positive that also blocks closures from the Slice-1 fuzzer. **Mechanizing borrow-escape for closures is the first thing Slice 3's oracle should add beyond this draft.**
...
- **Slice 3 (oracle).** Implement `classify` (§4) and a per-place-per-point state computation (§1–§3) as a standalone pass. It answers, for every place at every point: state (§1), and — for each program point — the exact set of drops to fire (§3.5) and which sub-places are split-suppressed (§3.4). The fuzzer then runs **differentially**: model says "drop here / this is Moved"; check codegen did the same. A divergence is now attributable to the *lowering*, not the model.
- **Slice 4 (codegen reads the oracle).** Replace the suppressor scatter (§3.4's list) and the drop-point re-derivation with reads of the oracle's facts. "Checker thinks it's Moved, codegen still drops it" becomes impossible by construction — one computed set of facts, both surfaces consult it. Land behind the Slice-1 fuzzer as the standing gate.
