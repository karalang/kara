...
- **Next — the consumption itself.** Everything is now in place to route one drop-decision (move-into-container suppression) through the oracle, gated by the ASan/LSan fuzzer (now conditional-move-aware). That is the remaining structural surgery — the one change that actually removes codegen's re-derivation — and the point where the risk finally lands on the drop path.
