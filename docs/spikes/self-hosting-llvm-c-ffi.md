...
- [ ] **`CStr.from_ptr(*const u8)` unsafe constructor** (or `String.from_c_ptr`). *Refinement surfaced by sub-q 4 (String marshaling).* The outbound read path (LLVM runtime-owned `char*` → Kāra `String`, then dispose) wants this; design.md:4638 describes the runtime-constructed-`CStr` *behavior* (O(N) `len` to NUL) but never names the constructor. Part of the Phase-8 *String marshaling* / `CStr`/`CString` type-body item, **not** a separate blocker — the read is hand-rollable today (manual strlen over `ptr.const` + `String.from_raw_parts`), so it gates ergonomics, not the port. → folds into [phase-12 § Cluster 2 FFI surface](../implementation_checklist/phase-12-self-hosting.md#pre-pivot-blockers-fix-before).
...
**Status:** decision record ✅ COMPLETE — all of sub-q 1–6 above resolved ([surface inventory](self-hosting-llvm-c-surface.md), linking, handles, marshaling, errors, bootstrapping). The minimal proof is **written and ready** as a lift-and-run spec — [`self-hosting-llvm-c-proof.md`](self-hosting-llvm-c-proof.md). The hard prerequisite — the `kara.toml [link]` directive — **landed 2026-06-11** (see Prerequisites above), so `libLLVM-18` is now linkable. **One prerequisite remains: `CStr.from_ptr`** (the soft one — used by the proof's `read_and_dispose`; hand-rollable from `ptr.const` + `String.from_raw_parts`). The spike closes the moment the proof runs green (`exit=42`) under both bootstrap stages; only `CStr.from_ptr` (or a hand-rolled read) stands between here and that run.
